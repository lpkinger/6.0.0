Ext.define('erp.view.hr.kbi.showKbi',{ 
    ...
    initComponent : function(){
        ...
        this.getGridColumnsAndStore(this, 'hr/kbi/singleGridPanel.action', gridParam, "");//从后台拿到gridpanel的配置及数据
        	...
        	callback : function(options,success,response){
        		...
        		if(1==1){
            		...
            		if(res.dbfinds&&res.dbfinds.length > 0){
            		 ...
            		 Ext.each(gridcolumns, function(column, y){
          				//power
          				if(limitArr.length > 0 && Ext.Array.contains(limitArr, column.dataIndex)) {
          					column.hidden = true;
          				}
          				if(column.gridcolumns.length>0){
          					column.columns=column.gridcolumns;
          				}
          				//renderer
