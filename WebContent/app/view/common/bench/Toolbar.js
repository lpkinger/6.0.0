Ext.define('erp.view.common.bench.Toolbar', {
	extend: 'Ext.toolbar.Paging',
	alias: 'widget.erpBenchGridToolbar',
	id:'BenchGridToolbar',
	doRefresh:function(){
		this.ownerCt.getColumnsAndStore(null, page, pageSize);
	},
	beforePageText : "第",
  	...
  	emptyMsg       : '没有数据',
	cls: 'u-toolbar',
	style:'border-top:1px solid #bdbdbd !important;',
	items: ['-',{
		name: 'export',
		...
		handler: function(btn){
			...
			Ext.create('Ext.window.Window', {
				...
				buttons : [{
					...
					handler : function(btn) {
						var panel = btn.ownerCt.ownerCt.down('erpResultGridPanel');
	...
	exportData : function(grid, btn, title, customFields) {
		if(!btn.locked) {
			...
			title = businesses.layout.getActiveItem().down('erpSwitchButton').getActive().text;
			if(title=='全部'){
				title = "";
			}else{
				 title += Ext.Date.format(new Date(), 'Y-m-d H:i:s');
			}
			grid.BaseUtil.createExcel(Scene, 'scenegrid', grid.getCondition(), title, null, customFields);
		}
	},
	updateInfo : function(){
		...
		if (displayItem) {
			...
			//修改表头全选checkbox选中状态
			var grid = me.ownerCt;
			if(grid.getSelectionModel().getCount()!=grid.store.getCount()){
				var hd_checker = grid.getEl().select('div.x-column-header-checkbox');
		...
		if (k == s.RETURN) {
			...
			if (pageNum !== false) {
				...
				if(me.fireEvent('beforechange', me, pageNum) !== false){
					page = pageNum;
					grid.getColumnsAndStore(null, page, pageSize);
				}
			}
	...
	moveFirst : function(){
		...
		value = 1;
		page = value;
		me.ownerCt.getColumnsAndStore(null, page, pageSize);
		me.updateInfo();
		fn(me,value);
	},
		...
		if(grid.noCount) {
			...
			if(!m) {
				m = Ext.create('Ext.panel.Tool', {
					...
					margin: '0 5 0 5',
					handler: function() {
						grid.getColumnsAndStore(null, null, true);
						m.hide();
						grid.noCount = false;
					}
				});
