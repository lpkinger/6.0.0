Ext.define('erp.view.pm.make.WCPlanMaterialLackGrid',{ 
   ...
   /* plugins: [Ext.create('Ext.ux.grid.GridHeaderFilters')],*/
    initComponent : function(){
        ...
        this.getGridColumnsAndStore(this, 'pm/make/singleGridPanel.action', gridParam, "");//从后台拿到gridpanel的配置及数据
	...
	loadNewStore:function(grid, url, param, no){
		Ext.Ajax.request({//拿到grid的columns
        	...
        	callback : function(options,success,response){
    	        ...
    	        var len = data.length;    	              	      
    	        for(var i=0;i<len;i++){
    	            if(Map[data[i].wd_makecode] == undefined){ 
		...
		Ext.Ajax.request({//拿到grid的columns
        	...
        	callback : function(options,success,response){
        		...
        		if(res.columns){
                    ...
                    startdate=Ext.Date.add(new Date(startdate), Ext.Date.DAY, -1);
                    enddate=Ext.Date.add(new Date(enddate), Ext.Date.DAY, -1);
                    if(startdate!=null){
                    	var lastcolumns=[];
                    	 Ext.Array.each(gridcolumns,function(item,m){
 	    		 			if(item.dataIndex=='ml_begindate'){
 	    		 				var columns=new Array();
 	    		 				var count=(enddate-startdate)/86400000;
 	    		 				for(var i=0;i<count+1;i++){
 	    		 					var field=Ext.Date.format(Ext.Date.add(startdate,Ext.Date.DAY,i),'Y-m-d');
 	    		 					gridfields.push({
 	    		 						name:field,
 	    		 						type:'int'
 	    		 					});
