Ext.define('erp.view.pm.make.WCPlanGrid',{ 
	extend: 'Ext.grid.Panel', 
	alias: 'widget.erpWCPlanGrid',
	...
	loadNewStore:function(grid, url, param, no){
		Ext.Ajax.request({//拿到grid的columns
        	...
        	callback : function(options,success,response){
    	        ...
    	        var len = data.length; 
    	        startdate=new Date(data[0].wc_fromdate);
    	        enddate=new Date(data[0].wc_todate);           	      
    	        for(var i=0;i<len;i++){
            		...
            		if(!res.data || res.data.length == 2){
            	        ...
            	        for(var i=0;i<len;i++){
            	            if(Map[data[i].wd_makecode] == undefined){ 
            	            	...
            	            	var smalldata=Map[data[i].wd_makecode];
            	            	smalldata[data[i].wd_date.substring(0,10)]=data[i].wd_planqty;
            	            	smalldata.wd_planqty+=data[i].wd_planqty;
            	                Map[data[i].wd_makecode]=smalldata;  
            	            }            	                	     
            	        } 
            	        for(var property in Map){
