Ext.define('erp.view.crm.chance.ChanceManage',{ 
   ...
   /* plugins: [Ext.create('Ext.ux.grid.GridHeaderFilters')],*/
    initComponent : function(){
        ...
        this.getGridColumnsAndStore(this, 'crm/Chance/singleGridPanel.action', gridParam, "");//从后台拿到gridpanel的配置及数据
	...
	loadNewStore:function(grid, url, param, no){
		Ext.Ajax.request({//拿到grid的columns
        	...
        	callback : function(options,success,response){
        		...
        		var data2 = Ext.decode(res.data2.replace(/,}/g, '}').replace(/,]/g, ']'));
        		  Ext.Array.each(data,function(item){
       		       Ext.Array.each(data2,function(item2){
       		    	   if(item.ch_code==item2.chcode){
       		    		   item[item2.name]=item2.result;
    	        ...
    	        var len = data.length;    	              	      
    	        for(var i=0;i<len;i++){
    	            if(Map[data[i].ch_code] == undefined){ 
    	            	var smalldata=data[i];
    	            	var startfield=Ext.Date.format(new Date(startdate),'Y-m-d');
    	            	var endfield=Ext.Date.format(new Date(enddate),'Y-m-d');
    	            	if(new Date(smalldata.ml_begindate)<new Date(startdate)){
    	            		smalldata[startfield]=smalldata.ml_mrponhandlack;
    	            	}else if(new Date(smalldata.ml_begindate)>new Date(enddate)) smalldata[endfield]=smalldata.ml_mrponhandlack;
    	            	else smalldata[smalldata.ml_begindate.substring(0,10)]=smalldata.ml_mrponhandlack;
    	                Map[data[i].ml_makecode] =smalldata;             	              
    	            }else{
    	            	var smalldata=Map[data[i].ml_makecode];
    	            	smalldata[data[i].ml_begindate.substring(0,10)]=data[i].ml_mrponhandlack;
    	                Map[data[i].ml_makecode]=smalldata;  
    	            }            	    
            		...
            		if(!res.data || res.data.length == 2){
            		    ...
            		    Ext.Array.each(data,function(item){
            		       Ext.Array.each(data2,function(item2){
            		    	   ...
            		    	   basecode=basecode==null?item2.chcode:basecode;           		    	 
            		    	   if(item2.chcode==basecode&&hasadd==false){
            		    		   gridcolumns.push({
   	    		 				     readOnly:false,	
   	    		 					 header:item2.name,
   	    		 					 cls: "x-grid-header-1",
   	    		 					 dataIndex:item2.name,
   	    		 					 width    : 60,
   	    		 					 align:'right',
   	    		 					 format:'0',
   	    		 				 	 renderer:'chancestatus' 
	    		 					});
            		    		   gridfields.push({
            		    			  name:item2.name,
            		    			  type:'string'
            	        ...
            	        var len = data.length;            	           	      
            	        for(var i=0;i<len;i++){
            	            if(Map[data[i].ml_makecode] == undefined){ 
            	            	var smalldata=data[i];
            	            	...
            	            	var smalldata=Map[data[i].ml_makecode];
            	            	smalldata[data[i].ml_begindate.substring(0,10)]=data[i].ml_mrponhandlack;
            	            	smalldata.ml_mrponhandlack+=data[i].ml_mrponhandlack;
            	                Map[data[i].ml_makecode]=smalldata;  
            	            }            	                	     
            	        } 
            	        for(var property in Map){
