 ...
 * 转借货归还单
 */	
Ext.define('erp.view.core.button.TurnYPOutReturn',{ 
		extend: 'Ext.Button', 
		alias: 'widget.erpTurnYPOutReturnButton',
		...
		listeners: {
			...
			click: function(m){
				console.log(m);
        		Ext.getCmp('turnypoutreturn').turn('ProdInOut!GoodsReturn!Deal', 
        				' pd_piid=' + Ext.getCmp('pi_id').value +' and nvl(pd_yqty,0) <nvl(pd_outqty,0) order by pd_pdno',
        				'scm/reserve/turnYPOutReturnnew.action?type=ProdInOut!GoodsReturn');
        	}
		},
