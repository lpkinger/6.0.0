Ext.define('erp.view.core.form.ItemSelector1', {
	extend: 'Ext.ux.form.MultiSelect',
	alias: ['widget.itemselectorfield1', 'widget.itemselector1'],
	...
	onAddBtnClick : function() {
		...
		Ext.Array.each(selected,function(item){
			if(item.data.value1.indexOf("org") != -1){
				me.setLoading(true);
				var id=item.data.value1.substring(4,item.data.value1.length);
				Ext.Ajax.request({//拿到tree数据
					url : basePath +'hr/hrorg/getEmployees.action',
					async :  false,
					params: {id:id},
					callback : function(options,success,response){
					var res = new Ext.decode(response.responseText);
						me.setLoading(false);
						item.data.value1=res.emp.value;
						item.data.value=res.emp.value1;
					}
				});
			}else if(item.data.value1.indexOf("job") != -1){
				me.setLoading(true);
				var id=item.data.value1.substring(4,item.data.value1.length);
				Ext.Ajax.request({//拿到tree数据
					url : basePath +'hr/job/getEmployees.action',
					async :  false,
					params: {id:id},
					callback : function(options,success,response){
					var res = new Ext.decode(response.responseText);
			...
			var d=item.data.value1.split(',');
			var v=item.data.value.split(',');
			for(var i=0;i<d.length;i++){
				if(d[i]!=''&&v[i]!=''){
					needto[v[i]]={ text:d[i], value:v[i] ,value1:d[i]};
				}	
			}
		});
