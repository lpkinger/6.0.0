 ...
 * 只针对作废出入库单据的grid
 */
Ext.define('erp.view.core.grid.CancelGrid',{ 
	extend: 'Ext.grid.Panel', 
	alias: 'widget.erpCancelGridPanel',
	...
	initComponent : function(){
		if(!this.boxready) {
			...
			this.GridUtil.getGridColumnsAndStore(this, 'scm/getCancelGridItems.action', gridParam, "");//从后台拿到gridpanel的配置及数据
		}
	...
	prev: function(grid, record){
		grid = grid || Ext.getCmp('cancelgrid');
				...
				if(r.buttons){
					...
					if(tb) {
						Ext.each(buttons, function(b){
							try {
								if(b.xtype.indexOf('erpCommonqueryButton!')!=-1){
									xtype = b.xtype.split('!')[0];
									id = b.xtype.split('!')[1];
									tb.add({
										xtype: xtype, 
										id : id,
