...
Ext.define('erp.view.core.grid.Panel2',{ 
	...
	/*listeners:{
		beforecollapse: function(grid){
			//收缩前记录原始高度
			grid._oldHeight = grid.el.dom.clientHeight;
		},
		collapse: function(grid,e){
			//缩小
			grid.setHeight(28);
			//viewport子项
			var items = grid.ownerCt.items.items;
			//针对主从表 后期处理多从表
			if(items.length==2&&items[0].xtype.indexOf('Form')>0){
				var form = items[0];
				form._oldHeight = form.el.dom.clientHeight;
				//放大主表
				form.setHeight(grid.ownerCt.height-28);
			}
		},
		expand: function(grid,e){
		    //还原
