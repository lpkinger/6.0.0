Ext.define('erp.view.vendbarcode.vendpurchase.VendToolbar', {
	extend: 'Ext.toolbar.Paging',
	alias: 'widget.erpVendDatalistToolbar',
	doRefresh:function(){
		...
		if (k == s.RETURN) {
			...
			if (pageNum !== false) {
				...
				if(me.fireEvent('beforechange', me, pageNum) !== false){
					page = pageNum;
					grid.getData(grid,getUrlParam('whoami'),condition,page,pageSize,page);
				}
			}
	...
	moveFirst : function(){
		...
		page = value;
		caller=grid.caller||caller;
		me.ownerCt.getData(grid,getUrlParam('whoami'),grid.getCondition(),page,pageSize,page);
		me.updateInfo();
		fn(me,value);
	},
	...
	afterOnLoad : function(num) {
		...
		d = e.getPageData();
		dataCount=this.ownerCt.store.dataCount||dataCount;
		b = d.currentPage;
		c = Math.ceil(dataCount / pageSize);
		a = Ext.String.format(e.afterPageText, isNaN(c) ? 1 : c);
		e.child("#afterTextItem").setText(a);
