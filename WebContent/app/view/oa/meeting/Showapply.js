Ext.define('erp.view.oa.meeting.Showapply',{ 
    ...
    initComponent : function(){
        ...
        this.getGridColumnsAndStore(this, 'oa/meeting/singleGridPanel.action', gridParam, "");//从后台拿到gridpanel的配置及数据
        		...
        		var data2 = Ext.decode(res.data2.replace(/,}/g, '}').replace(/,]/g, ']'));
        		  Ext.Array.each(data,function(item){
       		       Ext.Array.each(data2,function(item2){
       		    	 if(item.mr_code==item2.mamrcode){
            		...
            		if(!res.data || res.data.length == 2){
            		    ...
            		    Ext.Array.each(data,function(item){
            		       Ext.Array.each(data2,function(item2){
            		    	   ...
            		    	   basecode=basecode==null?item2.mamrcode:basecode;           		    	 
            		    	   if(item2.mamrcode==basecode&&hasadd==false){
            		    		   gridcolumns.push({
   	    		 				     readOnly:false,	
   	    		 					 header:item2.name,
   	    		 					 cls: "x-grid-header-1",
