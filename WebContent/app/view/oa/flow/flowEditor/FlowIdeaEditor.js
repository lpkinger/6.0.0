Ext.define('erp.view.oa.flow.flowEditor.FlowIdeaEditor',{ 
	extend: 'Ext.Viewport', 
	layout: 'fit', 
	requires:['erp.view.oa.flow.flowEditor.flowGrid.FlowIdeaGrid'],
	hideBorders: true, 
	initComponent : function(){
		...
		Ext.Ajax.request({
			...
			async: false,
			params:{
				fields : 'fo_groupname,fo_remark,fo_isduty',
			...
			items: [{ 
				...
				bbar: ['->',{
					...
					handler:function(btn){
						...
						var isDuty = Ext.getCmp('isDuty').value;
						if(!isDuty){
							showInformation('请填写派生意见的操作角色', function(btn){});
							return;
						}
						...
						//剔除重复field
						var deleteFields = Ext.getCmp('FlowIdeaGrid').nowFields.concat();//需要删除的数组
						...
						Ext.Ajax.request({
							url : basePath + 'oa/flow/saveUpdateOperation.action',
							params: {
								isDuty:isDuty,
			   					shortName:shortName,
			   					name:name,
				...
				items: [{
					...
					items:[{
						...
						store: Ext.create('Ext.data.Store',{
							...
							data:[{value:"true", display:"责任人"},{value:"false", display:"参与人"}]
					 	})
					},{
						fieldLabel: '分组名称',
						id: 'tabName',
