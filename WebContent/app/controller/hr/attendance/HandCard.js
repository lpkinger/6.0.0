Ext.QuickTips.init();
Ext.define('erp.controller.hr.attendance.HandCard', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    views:[
    		'hr.attendance.HandCard','core.form.Panel','core.form.FileField','core.form.MultiField','core.grid.Panel2',
  			...
  			'core.button.Flow','core.button.DownLoad','core.button.Scan','common.datalist.Toolbar',
  			...
  			'core.button.InsertEmployee','hr.attendance.EmpTree2'
  	],
	init:function(){
		var me = this;
		this.control({
			...
			'erpDeleteButton' : {
				afterrender: function(btn){
					var status = Ext.getCmp('hc_statuscode');
					if(status && status.value != 'ENTERING'){
						btn.hide();
					}
				},
				click: function(btn){
					me.FormUtil.onDelete(Ext.getCmp('hc_id').value);
				}
			},
			'erpUpdateButton': {
				afterrender: function(btn){
			...
			'erpAddButton': {
				click: function(){
					me.FormUtil.onAdd('addHandCard', '新增手工打卡单', 'jsps/hr/attendance/handcard.jsp');
				}
			},
			'erpCloseButton': {
				click: function(btn){
			...
			'erpAuditButton': {
				...
				click: function(btn){
//					me.FormUtil.onAuditWithManAndTime(Ext.getCmp('hc_id').value,'wo_auditer','wo_auditdate');
					me.FormUtil.onAudit(Ext.getCmp('hc_id').value);
				}
			}
    	});
	},
	
	insertEmployee: function (btn){
		
		var deptcode = Ext.getCmp('hc_deptcode').value;
		var hcid = Ext.getCmp('hc_id').value;
		if(deptcode != null && deptcode != '' && deptcode.length > 0 && hcid != null && hcid > 0){
			
			Ext.Ajax.request({
	        	url : basePath + '/hr/attendance/insertEmployee.action',
	        	params: {
	        		deptcode:deptcode,
	        		hcid:hcid
	        		},
