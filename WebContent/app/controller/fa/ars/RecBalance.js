Ext.QuickTips.init();
Ext.define('erp.controller.fa.ars.RecBalance', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','fa.ars.RecBalance','core.grid.Panel2','core.toolbar.Toolbar',
      		...
      		'core.button.AssDetail', 'core.trigger.MultiDbfindTrigger', 'core.form.SeparNumber','core.button.GetSumAmount','core.form.SeparNumber'
      	],
    init:function(){
    	var me = this;
    	...
    	this.control({
    		'erpGridPanel2': { 
    			afterrender: function(btn) {
    				var arr = ['RecBalance!PBIL','RecBalance!IMRE'];
    				if(Ext.Array.contains(arr, caller)){
    		...
    		'erpSaveButton': {
    			click: function(btn){
    				...
    				if(caller == 'RecBalance!PBIL' || caller == 'RecBalance!IMRE'|| caller == 'RecBalance!ARRM'){
    					this.getAramount();
    				}
    				//保存之前的一些前台的逻辑判定
    		...
    		'erpAddButton': {
    			click: function(){
    				...
    				me.FormUtil.onAdd('addRecBalance', title, 'jsps/fa/ars/recBalance.jsp?whoami='+caller);
    			}
    		},
    		'erpCloseButton': {
    			click: function(btn){
    		...
    		'#bi_code':{
    			beforetrigger: function(t){
    				...
    				if(!Ext.getCmp("rb_custcode")) {
    					showError("请先选择客户!");
    					return false;
    				} else if (!Ext.getCmp("rb_cmcurrency")){
    					showError("请先选择冲账币别!");
    					return false;
    				} else {
    			...
    			click: function(btn){
    	    		...
    	    		Ext.each(items,function(item,index){
    	    			if(!me.GridUtil.isBlank(grid,item.data)) {
    	    				detailamount1 = detailamount1 + Number(item.data['rbd_nowbalance']);
    	    			}
    	    		});
    	    		Ext.getCmp("rb_amount").setValue(detailamount1);
    	    		Ext.getCmp("rb_cmamount").setValue(detailamount1);
    			}
    		},
    		'erpResSubmitButton': {
    		     afterrender: function(btn){
    		...
    		'button[name=catchab]':{
    			...
    			click:function(btn){
					...
					if(accode&&!accode.isHidden()){
       			    ...
       			    startdate=toolbar.items.items[5].value;
       			    enddate=toolbar.items.items[7].value;
    				Ext.each(items,function(item,index){
    					if(item.data[grid.keyField] != null && item.data[grid.keyField] != ''){
    						array.push(item);
    					}
    				});
    		...
    		'field[name=rb_date]':{
				beforerender: function(field){
					if(Ext.getCmp('rb_source') && (Ext.getCmp('rb_source').value=="Bank" || Ext.getCmp('rb_source').value=="应收票据" || Ext.getCmp('rb_source').value=="应收票据退票")){
						field.readOnly=true;
					}
				}
			}
    ...
    beforePost:function(btn){
    	...
    	if(caller == 'RecBalance!PBIL'||caller == 'RecBalance!ARRM'||caller == 'RecBalance!IMRE'){
    		var grid = Ext.getCmp('grid'),items=grid.store.data.items;
    		...
    		if(amount != 0 && cmamount != 0){
	    		if(form.BaseUtil.numberFormat(cmamount,2) != form.BaseUtil.numberFormat(form.BaseUtil.multiply(cmrate, amount),2)){
					showError('冲账汇率不正确,不能过账!');
					return;
				}
    		}
    		if(currency == cmcurrency){
				if(amount != 0 && cmamount != 0 && cmrate != 1){
    ...
    changecmrate: function(){
    	if(Ext.getCmp('rb_amount') && Ext.getCmp('rb_cmrate')) {
    		...
    		if(rbamount != 0)
    			Ext.getCmp('rb_cmrate').setValue(Ext.Number.toFixed(cmamount/rbamount, 15));
    	}
    },
	...
	beforeUpdateRecBalance: function(){
		...
		var items = grid.store.data.items;
		var bool = true;
		var warnStr='', cmCurr = Ext.getCmp('rb_cmcurrency').getValue();
		Ext.each(items,function(item,index){
			if(item.dirty && item.data[grid.necessaryField] != null && item.data[grid.necessaryField] != ''){
				...
				if(Ext.isEmpty(item.get('rbd_currency'))) {
					item.set('rbd_currency', cmCurr);
				}
			}
		});
		if(bool){
