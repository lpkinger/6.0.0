Ext.QuickTips.init();
Ext.define('erp.controller.fa.gla.GeneralLedgerMulti', {
    extend: 'Ext.app.Controller',
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views: ['core.trigger.DbfindTrigger', 'fa.gla.GeneralLedgerMulti', 'fa.gla.LedgerMulti', 'core.form.MonthDateField', 
    ...
    init:function(){
    	...
    	this.control({ 
    		...
    		'button[name=export]': {
    			click: function() {
    				var grid = Ext.getCmp('ledgerMulti');
    ...
    query: function(cond) {
    	...
    	Ext.Ajax.request({
    		url: basePath + 'fa/ars/getGeneralLedgerMulti.action',
    		params: {
    		...
    		callback: function(opt, s, r) {
    			...
    			if(grid && res.data) {
    				...
    				var amm_assmulti = grid.down('gridcolumn[dataIndex=amm_assmulti]');
    				if(cond.chkall) {
    					if(amm_assmulti.hidden)
    						amm_assmulti.show();
    				} else {
    					if(!amm_assmulti.hidden)
    						amm_assmulti.hide();
    ...
    loadDetail: function(store, record) {
    	var ym = record.get('cm_yearmonth'),
    		...
    		ac = record.get('amm_acid'),
    		un = Ext.getCmp('chkhaveun').value;
    	this.BaseUtil.onAdd('GLDetailMulti_' + ym + '_' + caCode, '明细账', 
    			'jsps/fa/gla/glDetailMulti.jsp?y=' + ym + '&c=' + caCode + '&cr=' + cr + '&ac=' + ac + '&un=' + un);
    },
