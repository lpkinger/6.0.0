Ext.QuickTips.init();
Ext.define('erp.controller.fa.gla.ChildReport', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','fa.gla.ChildReport','core.grid.Panel2','core.toolbar.Toolbar',
      		'core.button.Save','core.button.Add','core.button.Submit','core.button.ResAudit',
      		'core.button.Audit','core.button.Update','core.button.DeleteDetail','core.button.ResSubmit',
	...
	hideColumn: function(fatype, change){
		...
		if(fatype != '资产负债表'){
			...
			grid.down('gridcolumn[dataIndex=crd_rightamount1]').hide();
			grid.down('gridcolumn[dataIndex=crd_rightamount2]').hide();
			grid.down('gridcolumn[dataIndex=crd_rightbbamount1]').hide();
			grid.down('gridcolumn[dataIndex=crd_rightbbamount2]').hide();
			grid.down('gridcolumn[dataIndex=crd_amount1]').setText('本月数');
			grid.down('gridcolumn[dataIndex=crd_amount2]').setText('本年累计数');
			grid.down('gridcolumn[dataIndex=crd_bbamount1]').setText('本币本月数');
			grid.down('gridcolumn[dataIndex=crd_bbamount2]').setText('本币本年累计数');
		} else {
			grid.down('gridcolumn[dataIndex=crd_rightitem]').show();
			grid.down('gridcolumn[dataIndex=crd_rightstep]').show();
			grid.down('gridcolumn[dataIndex=crd_rightamount1]').show();
			grid.down('gridcolumn[dataIndex=crd_rightamount2]').show();
			grid.down('gridcolumn[dataIndex=crd_rightbbamount1]').show();
			grid.down('gridcolumn[dataIndex=crd_rightbbamount2]').show();
			grid.down('gridcolumn[dataIndex=crd_amount1]').setText('年初数');
			grid.down('gridcolumn[dataIndex=crd_amount2]').setText('年末数');
			grid.down('gridcolumn[dataIndex=crd_bbamount1]').setText('本币年初数');
			grid.down('gridcolumn[dataIndex=crd_bbamount2]').setText('本币年末数');
		}
	}
});