Ext.QuickTips.init();
Ext.define('erp.controller.pm.bom.ECRDetailLocation', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','pm.bom.ECRDetailLocation','core.grid.Panel2','core.toolbar.Toolbar',
		...
		this.control({
			...
			'erpDeleteButton' : {
				afterrender: function(btn){
					if(Ext.getCmp('ecrd_id').value != null && Ext.getCmp('ecrd_id').value != ''){
						btn.show();
					} else {
						btn.hide();
					}
				...
				click: function(btn){
					me.FormUtil.onDelete(Ext.getCmp('ecrd_id').value);
				}
			},
			'erpUpdateButton': {
				afterrender: function(btn){
			...
			'erpAddButton': {
				click: function(){
					me.FormUtil.onAdd('addECRDetailLocation', '新增ECR物料位号', 'jsps/pm/bom/ECRDetailLocation.jsp');
				}
			},
			'erpCloseButton': {
				click: function(btn){
			...
			'dbfindtrigger[name=ecrd_sonid]': {
    			afterrender: function(t){
    				t.dbKey = "ecrd_bomid";
    				t.mappingKey = "ecrd_ecrid";
    				t.dbMessage = "请先选择ECRID!";
    			}
    		},
			'field[name=ecrd_id]': {
				change: function(f){
