Ext.QuickTips.init();
Ext.define('erp.controller.pm.make.WCPlanMaterialLack', {
    extend: 'Ext.app.Controller',
    requires: ['erp.util.FormUtil', 'erp.util.GridUtil', 'erp.util.BaseUtil', 'erp.util.RenderUtil'],
    views:[
     		'common.batchDeal.Viewport','common.batchDeal.Form','pm.make.WCPlanMaterialLackGrid',
    	...
    	this.control({ 
    		...
    		'erpVastDealButton':{
    			...
    			click:function(btn){
    			   ...
    			   var arr=new Array();
    			   Ext.Array.each(grid.columns,function(gridcolumn){
    				dd = new Object();
    				   dd.dataIndex=gridcolumn.dataIndex;
    				   dd.xtype=gridcolumn.xtype;
    				   dd.text=gridcolumn.text;
    				   if(gridcolumn.columns){
                 		  Ext.Array.each(gridcolumn.columns,function(column){
                 			 dd=new Object();
                 			 dd.dataIndex=column.dataIndex;
          				     dd.xtype=column.xtype;
          				     dd.text=column.dataIndex;
                 			 arr.push(Ext.JSON.encode(dd)); 
                 		  });
                 	  }else arr.push(Ext.JSON.encode(dd));
    			   });
      		       var columns= unescape(arr.toString().replace(/\\/g,"%"));
      		       var param=jsonGridData;
      		       param=param=null?[]:'['+param.toString()+"]";
      		       var data=unescape(param.toString().replace(/\\/g,"%"));  
      		       window.location.href=basePath+'excel/twoExport.action?data='+data+"&columns="+columns;
    			}
    		 },
    		'button[id=query]':{    
    			beforerender:function(btn){
    				btn.handler=function(select){
    					...
    					if(grid.getGridColumnsAndStore){
    						grid.loadNewStore(grid, 'common/singleGridPanel.action', gridParam, "");
    					}
    					if(select == true) {
