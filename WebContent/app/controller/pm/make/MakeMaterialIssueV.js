Ext.QuickTips.init();
Ext.define('erp.controller.pm.make.MakeMaterialIssueV', {
	extend: 'Ext.app.Controller',
	requires: ['erp.util.RenderUtil', 'erp.util.GridUtil', 'erp.util.BaseUtil'],
	views: ['pm.make.MakeMaterialIssueV', 'core.grid.Panel6', 'common.editorColumn.GridPanel', 'core.grid.YnColumn', 'core.button.CreateDetail', 'core.button.PrintDetail', 'core.trigger.DbfindTrigger',
...
//					}
	...
	onQuery: function() {
		...
		var length = grid.getStore().getModifiedRecords().length;
        if(length != 0 && (c.length != length)){
			showError('筛选的单据编号重复');
			return ;
    	}
		grid.selModel.deselectAll(true);
		...
		if (condition != null) {
			...
			dg.busy = true;
			condition += " AND (nvl(mm_materialstatus,' ')=' ')  AND (mm_qty-nvl(mm_totaluseqty,0)-(nvl(mm_havegetqty,0)+nvl(mm_returnmqty,0)-nvl(mm_addqty,0)) > 0)";
		...
		Ext.Array.each(items,
			...
			if (item.get('mm_thisqty') != 0) {
				d.push({
					...
					mm_id: item.get('isrep') == '' ? item.get('mm_id') : -item.get('mm_id'),
