Ext.define('erp.controller.as.port.NewBack', {
    extend: 'Ext.app.Controller',
    requires: ['erp.util.FormUtil', 'erp.util.GridUtil', 'erp.util.BaseUtil', 'erp.util.RenderUtil'],
    views:[
     		'as.port.NewBack','core.button.VastDeal','core.button.Delete','core.button.Close'
     	],
    ...
    init:function(){
    	...
    	this.control({
    		...
    		'erpVastDealButton': {
    			click: function(btn){
    				me.vastDeal('as/port/newBackToProdIO.action');
    			}
    		},
    		'erpDeleteButton': {
    			click: function(btn){
    				me.vastDeal('as/port/newBackDelete.action');
    			}
			}
    	});
    },
    vastDeal: function(url){
    	...
    	if(records.length > 0){
    		Ext.each(records, function(record, index){
    			var v = record.data['NEWFITTINGBACK_NO'];
    			data.push(v);
    		});
