Ext.define('erp.controller.as.port.HeXiao', {
    extend: 'Ext.app.Controller',
    requires: ['erp.util.FormUtil', 'erp.util.GridUtil', 'erp.util.BaseUtil', 'erp.util.RenderUtil'],
    views:[
     		'as.port.HeXiao','core.button.VastDeal','core.button.Delete','core.button.Close'
     	],
    ...
    init:function(){
    	...
    	this.control({
    		...
    		'erpVastDealButton': {
    			click: function(btn){
    				me.vastDeal('as/port/hexiaoToProdIO.action');
    			}
    		},
    		'erpDeleteButton': {
    			click: function(btn){
    				me.vastDeal('as/port/hexiaoDelete.action');
    			}
			}
    	});
    },
    vastDeal: function(url){
    	...
    	if(records.length > 0){
    		Ext.each(records, function(record, index){
    			var v = record.data['APPLYHEXIAO_NO'];
    			data.push(v);
    		});
