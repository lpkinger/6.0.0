Ext.QuickTips.init();
Ext.define('erp.controller.b2c.sale.GoodsUpApplication', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
    		'b2c.sale.b2cPanel','b2c.common.b2cForm','b2c.common.Viewport','b2c.common.b2cGrid','core.toolbar.Toolbar','core.button.Split',
    	...
    	this.control({ 
    		'erpGridPanel2': { 
    			itemclick: function(selModel, record){
    				...
    				if(record.data.gd_id != 0 && record.data.gd_id != null && record.data.gd_id != ''){
    					var btn = Ext.getCmp('erpSplitButton');
						btn && btn.setDisabled(false);
    				}
    			}
    		},  		 
    				 ...
    				 Ext.Ajax.request({
					   	  ...
					   	  callback : function(options,success,response){
					   		...
					   		if(localJson.success){
					   			if(localJson.code)
					   			   me.FormUtil.onAdd('ProductBatchUUId','批量维护标准料号','jsps/b2c/component/b2cProductBatchUUId.jsp?gridCondition=pub_code='+localJson.code+'&code='+localJson.code);
    		...
    		'gridcolumn[dataIndex=gd_ismallsale]':{
    			afterrender:function(g){
    				me.BaseUtil.getSetting('B2CSetting','B2CISMallSale',function(val){
    					var grid = Ext.getCmp('grid');
    					grid.getStore().each(function(record){
    						var gd_ismallsale = record.data['gd_ismallsale'];
    						if(gd_ismallsale==''||gd_ismallsale==null){
    							record.set('gd_ismallsale', val); 
    						}
    					})
    				});
    			}
