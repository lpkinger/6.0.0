Ext.QuickTips.init();
Ext.define('erp.controller.b2c.sale.MRPOnHandThrow', {
    	...
    	this.control({ 
    		'erpBatchDealGridPanel': {
    			...
    			storeloaded:function(grid){
    				...
    				var reminder = Ext.getCmp('dealform').down('#reminder');
    				if(reminder){
    		...
    		'textfield[name=gu_currency]':{//币别取商城客户中的币别
    			afterrender:function(t){
    				this.BaseUtil.getSetting('sys','defaultCurrency',function(data){
    					t.setValue(data);
    					var gd_taxrate = Ext.getCmp("gd_taxrate");
    					if(data=='RMB'){
    						gd_taxrate.setValue(17);
    					}else{
    						gd_taxrate.setValue(0);
    					}
    				});
    			}
    		},
			        	...
			        	var res = new Ext.decode(response.responseText); 
			        	   if(res.exceptionInfo){ 
			        			...
			        			Ext.Msg.alert('提示',"运算成功",function(){      
							    	...
							    	window.location.href = basePath+'jsps/b2c/sale/MRPOnHandThrow.jsp?whoami=MRPOnhandThrow&_noc=1&urlcondition='+condition+'&mdd_mpscode='+res.message;
			        			});
			        		}
			        	}
			        });
	   		...
	   		callback : function(options,success,response){
    			...
    			if(localJson.success){
    				if(localJson.log){
    					showMessage("提示", localJson.log);
    					Ext.getCmp('dealform').onQuery();
	   					var url = "jsps/b2c/common/b2cDatalist.jsp?whoami=DeviceInApply";
