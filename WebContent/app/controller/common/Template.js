Ext.QuickTips.init();
Ext.define('erp.controller.common.Template', {
    				...
    				if(grid) {
    					...
    					/*if(f.checked) {
        					grid.store.filterBy(function(item) {
					    		return !Ext.isEmpty(item.get('log')); 
					    	});
        				} else {
        					grid.store.filterBy(function(item) {
					    		return true; 
	...
	getNode: function(record){
		var me = this,
			p = Ext.getCmp('template');
		me.getDetails(p, record.raw.caller, false);
	},
    ...
    getCheckResult: function(grid, p){
    	...
    	Ext.Ajax.request({
    		...
    		callback: function(opt, s, r){
    	    	...
    	    	Ext.getCmp('errdelete').show();
    	    	Ext.getCmp('saveupdates').show();
    	    	console.log(rs.indexOf('ac_date'));
    	    	if(rs.indexOf('ac_date')>0&&grid.caller=='AssetsCard'){
    	    		alert('卡片入账日期不能大于当前账期！');
    	    	}
    		}
    	});
    },
    ...
    saveUpdates: function(grid){
    	var items = grid.store.data.items,arr = new Array();
