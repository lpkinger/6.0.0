Ext.QuickTips.init();
Ext.define('erp.controller.common.Import', {
    ...
    init: function(){ 
    	...
    	this.control({ 
    					...
    					if(btn.grid){
    						btn.getForm().submit({
        		        		...
        		        		success: function(fp, o){
        		        			...
        		        			if(btn.grid.caller=='BOM'||btn.grid.caller=='BOM!Additional'||btn.grid.caller=='BOM!New'){
        		        				Ext.getCmp('matchingcode').show();
        		        			}
        		        		},
    			...
    			click: function(btn){
    				...
    				if(grid){
    					window.location = basePath + 'system/initTemplate.xls?caller=' + grid.caller + 
    						'&title=' + (encodeURI(encodeURI(grid.ownerCt.title.replace(/&raquo;/g, '/'))));
    				}
    			}
    		},
    		'button[id=rule]': {
    		...
    		'button[id=errdownload]': {
    			click: function(btn){
    				...
    				if(grid && grid.ilid > 0) {
						...
						'&title=ERR_' + (grid.ownerCt.title.replace(/&raquo;/g, '/') + '&id=' + grid.ilid);
    				}
    			}
    		},
    		'button[id=alldownload]': {
    			click: function(btn){
    				...
    				if(grid && grid.ilid > 0) {
						...
						'&title=INIT_' + (grid.ownerCt.title.replace(/&raquo;/g, '/') + '&id=' + grid.ilid);
    				}
    			}
    		},
    		'#template': {
    		...
    		'button[id=matchingcode]':{
    			click: function(btn){
    				var grid = btn.ownerCt.ownerCt.down('gridpanel');
    				if(grid && grid.down('pagingtoolbar').dataCount<=0){
    					showMessage('提示', '无数据');
    				}else if(grid){
    					Ext.Ajax.request({
				    		url: basePath + 'system/matchingCode.action',
				    		timeout: 300000,
				    		method: 'post',
				    		params: {
				    			id: grid.ilid,
				    		...
				    		callback: function(opt, s, r){
				    			...
				    			if(!rs){
				    				...
				    				if(rs.exceptionInfo.indexOf('AFTERSUCCESS')>=0){
				    					showError(rs.exceptionInfo);
				    				}else{
				    					Ext.Msg.alert('发现错误',rs.exceptionInfo);
				    					return;
				    				}
				    			}
				    			grid.getGridData(1);
    ...
    showHistory: function(caller){
    	var w = Ext.create('Ext.Window', {
    		...
    		items: [{
    			...
    			anchor: '100% 100%',
    			columnLines: true,
    			columns: [{dataIndex: 'il_id', hidden: true},{dataIndex: 'il_caller', hidden: true}, {dataIndex: 'il_man', text: '操作人', flex: 1},{dataIndex: 'il_date', text: '日期', flex: 2},
    			...
    			store: Ext.create('Ext.data.Store', {
    				fields: ['il_id', 'il_caller', 'il_date', 'il_sequence', 'il_count', 'il_checked', 'il_success', 'il_toformal','il_man'],
    				data: [{},{},{},{},{},{},{},{},{},{}]
    			})
    		}]
    	});
