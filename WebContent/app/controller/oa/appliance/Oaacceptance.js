Ext.QuickTips.init();
Ext.define('erp.controller.oa.appliance.Oaacceptance', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
    		'oa.appliance.Oaacceptance','core.form.Panel','core.grid.Panel2','core.toolbar.Toolbar','core.form.MultiField','core.form.FileField',
    ...
    init:function(){
    	...
    	this.control({ 
    		...
    		'erpAddButton': {
    			click: function(){
    				me.FormUtil.onAdd('addOaacceptance', '新增采购验收单', 'jsps/oa/appliance/oaacceptance.jsp');
    			}
    		},'erpSubmitButton': {
			...
			'erpPostButton': {
				afterrender: function(btn){
    				...
    				if(status && status.value != 'UNPOST'||Ext.getCmp('op_statuscode').value!='AUDITED'){
    					btn.hide();
    				}
    			},
    			buffer : 1000,
			...
			'erpResPostButton': {
				afterrender: function(btn){
    				var status = Ext.getCmp('op_inoutstatuscode');
    				if(status && status.value != 'POSTED'||Ext.getCmp('op_statuscode').value!='AUDITED'){
    					btn.hide();
    				}
    			},
    			click: function(btn){
