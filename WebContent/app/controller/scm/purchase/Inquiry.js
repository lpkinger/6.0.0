...
Ext.define('erp.controller.scm.purchase.Inquiry', {
    ...
    views:[
      		'core.form.Panel','scm.purchase.Inquiry','core.grid.Panel2','core.toolbar.Toolbar','core.form.FileField','core.button.Modify',
      		'core.button.Save','core.button.Update','core.button.Add','core.button.Submit','core.button.Print','core.button.Upload',
  			'core.button.Audit','core.button.Close','core.button.Delete','core.button.DeleteDetail','core.button.ResSubmit','core.button.AgreeAutoPrice',
  			...
  			'core.trigger.DbfindTrigger','core.trigger.TextAreaTrigger','core.form.YnField','core.button.TurnPurcPrice','core.button.AgreePrice','core.grid.YnColumn','core.button.SubmitAudit','core.button.ResSubmitAudit'
      	],
    init:function(){
    	var me = this;
    	me.alloweditor = true;
    	...
    	this.control({ 
    		...
    		'erpFormPanel':{
    			afterrender:function(){
    				me.BaseUtil.getSetting('Inquiry', 'allowAuditProcess', function(bool) {
...
//    						}
    		...
    		'erpSubmitButton': {
    			...
    			click: function(btn){
    				...
    				var url = 'scm/purchase/submitInquiry.action';
    				if(in_prodtype&&in_prodtype.value=='公开询价'){
    					url = 'scm/purchase/submitInquiry.action?caller=Inquiry';
    				}
    				if((inclass&&inclass.value=='主动报价') || (in_prodtype&&in_prodtype.value=='公开询价')){
    		...
    		'erpAgreePriceButton': {
    			...
    			click: function(btn){
    			   ...
    			   var	url = basePath+'jsps/scm/purchase/inquiryVastDatalist.jsp?_noc=1&whoami=ToPrice&urlcondition=id_inid='+inid+' and nvl(id_price,0)>0';	   
    			   var win =Ext.getCmp('Inquiry' + id);
    			   if(!win){
    				   win=Ext.create('Ext.window.Window', {
    					...
    					layout : 'anchor',
       		            ...
       		            items: [{ 
	    	    			...
	    	    			anchor : '100% 100%',
	    	    			layout : 'fit',
	    	    			html : '<iframe id="iframe_add_'+ id+'" src="' + url+'" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe>',
	    	    		}]
       		        });
    			   }
    			   win.show();   			   
    		...
    		'erpAgreeAutoPriceButton': {
    			...
    			click: function(btn){
    				...
    				for(var i=0;i<s.length;i++){//将grid里面各行的数据获取并拼成jsonGridData
				    	...
				    	var dd = new Object();
				    	dd["id_id"]=data["id_id"];
				    	dd["id_isagreed"]=data["id_isagreed"];
				    	if(dd["id_isagreed"]==true){
				    		isAgreedCount++;
				    	}
				    	param.push(Ext.JSON.encode(dd));
    				}
					param = param == null ? [] : "[" + param.toString().replace(/\\/g,"%") + "]";
					//判断是否有价格被采纳  lidy 2018030433
					if(isAgreedCount==0){
					...
					me.agreeAutoPrice(param,isAgreedCount);
    			}
    		},
    ...
    onGridItemClick: function(selModel, record){//grid行选择
    	...
    	if(show){
        	...
        	btn && btn.setDisabled(true);
        	btn = Ext.getCmp('stepWiseInquiry');
        	btn && btn.setDisabled(true);
	...
	copy: function() {
	 ...
	 var v = Ext.getCmp('in_id').value;
	 if (v > 0) {
	     form.setLoading(true);
	     Ext.Ajax.request({
	         url: basePath + 'scm/purchase/copyInquiry.action',
	         params: {
	             caller: caller,
	             id: v
	         },
	         callback: function(opt, s, r) {
	             ...
	             if (res.data) {
	                 var url = 'jsps/scm/purchase/inquiry.jsp?formCondition=in_idIS' + res.data.id + '&gridCondition=id_inidIS' + res.data.id;
	                 showMessage('提示', '复制成功', 2000);
	                 me.FormUtil.onAdd(null, '询价单', url);
	             } else {
	...
	getStepWise: function(in_id, callback) {
		Ext.Ajax.request({
			url: basePath + 'scm/purchase/inquiry/det.action',
			params: {
				in_id: in_id
			},
			callback: function(opt, s, r) {
	...
	updateInfo:function(purpose, remark, id){
		Ext.Ajax.request({
        	url : basePath + 'scm/purchase/inquiry/updateInfo.action',
        	params: {
        		purpose: purpose,
        		remark: remark,
        		id: id,
	...
	agreeAutoPrice: function(param,isAgreedCount){
		var me = this;
		Ext.Ajax.request({
	   		url : basePath + 'scm/purchase/agreeInquiryPrice.action',
	   		params: {
	   			id: Ext.getCmp('in_id').value,
	   			param: param
	   		},
