...
Ext.define('erp.controller.scm.purchase.Purchase', {
    ...
    views: ['core.form.Panel', 'scm.purchase.Purchase', 'core.grid.Panel2', 'core.toolbar.Toolbar', 'core.form.MultiField', 
    		'core.button.Save', 'core.button.Add', 'core.button.Submit', 'core.button.Print', 'core.button.PrintHK', 'core.button.PrintEn',
    		'core.button.Upload', 'core.button.ResAudit', 'core.button.Audit', 'core.button.Close', 'core.button.Delete', 
    		'core.button.Update', 'core.button.B2B','core.button.DeleteDetail', 'core.button.ResSubmit', 'core.button.End',
    		'core.button.Printyestax', 'core.button.Printnotax','core.button.AttendDataCom','core.button.ResEnd', 'core.button.GetPrice', 
    		'core.button.Export', 'core.button.StandardPrice','core.trigger.DbfindTrigger', 'core.trigger.TextAreaTrigger', 
    		'core.form.YnField', 'core.grid.YnColumn', 'core.form.StatusField', 'core.form.FileField', 'core.button.PrintByCondition',
    		'core.button.SendEdi','core.button.CopyAll', 'core.button.ResetSync', 'core.button.RefreshSync','core.button.RefreshQty',
    		'core.button.Split','core.button.DeliveryApply','core.button.ModifyDetail','core.button.TurnBankRegister2','core.button.Modify','core.button.HistoryQuotation',
    		'core.button.ContractProcess','core.button.Commonquery','core.button.updateGridDetailReplyDate','core.trigger.MultiDbfindTrigger'],
    init: function() {
        var me = this;
        this.control({
        	'erpFormPanel':{
...
//    		},
                ...
                click: function(btn) {
                	warnMsg("确定结案?", function(btn){
    					if(btn == 'yes'){
    	    				...
    	    				Ext.Ajax.request({
    	    			   		url : basePath + 'scm/purchase/endPurchase.action?id='+Ext.getCmp('pu_id').value,
    	    			   		params: {
    	    			   			caller: caller,
            ...
            'erpDeliveryApplyButton' : {
            	click : function(btn){
            		var record = btn.ownerCt.ownerCt.getSelectionModel().getLastSelected();
                    me.getDeliveryApplyWindow(record);
            	}
            },
            'erpPrintButton': {
                click: function(btn) {
    		...
    		'button[id=historyquo]':{
    			click : function(btn){
    				...
    				if(record){
    					var pd_prodcode = record.data.pd_prodcode;
    					btn.getWin(pd_prodcode);
    				}
    			}
    		},
    		'erpContractProcess': {
    			afterrender: function(btn){
    				var source = Ext.getCmp('pu_source');
    				if(!source||source.value!='招投标'){
    					btn.hide();
    				}
    			}
    		},
    		'#updateGridDetailReplyDate': {
    			afterrender: function(btn){
    				var status = Ext.getCmp('pu_statuscode').value;
    				console.log(status);
    				if(status!='COMMITED'&&status!='AUDITED'){
    					btn.hide();
    				}
    			},
                click: function(btn) {
    ...
    saveReplyDate: function(w,id){
        ...
        if (!deliveryreply) {
    		...
    		params.id = id;
    		params.date = unescape(Ext.Date.format(deliveryreply, 'Y-m-d').toString().replace(/\\/g,"%"));
            console.log(params);
            Ext.Ajax.request({
                url: basePath + 'scm/purchase/updateGridDetailReplyDate.action',
                params: params,
                method: 'post',
                callback: function(opt, s, res) {
                    var r = new Ext.decode(res.responseText);
    ...
    onGridItemClick: function(selModel, record) { //grid行选择	
        if (record.data.pd_id != 0 && record.data.pd_id != null && record.data.pd_id != '') {
        	...
        	btn = Ext.getCmp('erpDeliveryApplyButton');
        	if(status=='FINISH'){
        		btn.setDisabled(true);
        	}else{
        		btn && btn.setDisabled(false);
    ...
    beforeSavePurchase: function() {
        ...
        function(item) {
        	item.data['pd_code']=c;
        	item.data['pd_vendid']=vend;
        	item.data['pd_vendcode']=vendcode;
        	item.data['pd_vendname']=vendname;
        });
        //手工录入采购单,合同类型不能为标准
        ...
        Ext.each(items, function(item) {
            if (item.dirty && item.data[grid.necessaryField] != null && item.data[grid.necessaryField] != "") {
                if (item.data['pd_delivery'] == null) {
                	if(Ext.getCmp('pu_delivery')&&Ext.getCmp('pu_delivery').value!=null&&Ext.getCmp('pu_delivery').value!=""){
                		item.set('pd_delivery', Ext.getCmp('pu_delivery').value);
                	}
    ...
    getDeliveryApplyWindow : function(record){
    	var me = this;
		var pucode=record.data.pd_code;
		var pddetno=record.data.pd_detno;
		var qty = record.data.pd_qty-record.data.pd_acceptqty;
    	Ext.create('Ext.window.Window',{
    		...
    		items : [{
    			...
    			defaults:{
    				...
    				columnWidth:0.5,
    				readOnly:true,
    				fieldStyle:'background:#f0f0f0;border: 1px solid #bdbdbd;color:blue;',
    				labelAlign:'right'
    			},
    			items : [{
    				fieldLabel:'物料编号',
        		...
        		buttons : [{
    				...
    				handler:function(btn){
    					...
    					var flag = false;
    					Ext.Array.each(items,function(item){
    						if(item.data.pr_delivery==null || item.data.pr_qty<=0){
    							flag = true;
    						}
    						arr.push(item.data);
    					});
    					if(flag==true){
    						showError("回复交期不能为空、回复数量需大于0！");
    						return;
    					}
    					grid.setLoading(true);
    					Ext.Ajax.request({
    						url:basePath+'scm/purchase/dataReply.action',
    						params :{
    							pucode : record.data.pd_code,
    							detno : record.data.pd_detno,
    							qty : qty,
    			...
    			store : Ext.create('Ext.data.Store',{
    				fields : [
    				    {name:'pr_qty',type:'int'},
    				    {name:'pr_delivery',type:'date'},
    				    {name:'pr_remark',type:'string'}
    				],
    				data : []
    			}),
    		    ...
    		    tbar: [{
	  	            ...
	  	            width:25,
	  	            handler : function() {
	  	            	var store = Ext.getCmp('applygrid').getStore();
	  	                var r = new Object();
	  	                r.pr_delivery=record.get('pd_delivery');
	  	                r.pr_qty=0;
	  	                r.pr_remark='',
	  	            ...
	  	            handler: function(btn) {
	  	                var sm = Ext.getCmp('applygrid').getSelectionModel();
    		...
    		items:[{
    			...
    			buttons:[{
    				...
    				handler:function(btn){
    				   ...
    				   var dd;
    				   var qty=false;//pd_qtyreply
    				   Ext.Array.each(store.data.items,function(item){
    					  if(item.data.pd_delivery!=null&&item.data.pd_qty>0){
    						  if(item.dirty){
    							  dd=new Object();
    							  //说明是新增批次
    							  ...
    							  if(item.data.pd_deliveryreply){
    								  dd['pd_deliveryreply']=Ext.Date.format(item.data.pd_deliveryreply, 'Y-m-d H:i:s'); 
    							  }else
    								  dd['pd_deliveryreply']=null;
    							  if(item.data.pd_sellercode){
    								  dd['pd_sellercode']=item.data.pd_sellercode; 
    							  	  dd['pd_seller']=item.data.pd_seller; 
    							  }else{
    							  	 dd['pd_sellercode']==null;
    							  	 dd['pd_seller']==null;
    							  }
    					  ...
    					  if(item.data.pd_qty==0 || item.data.pd_qtyreply<0){
							  qty = true;
						  }
    				   });		   
    				   var assqty=Number(record.data.pd_qty);
    				   if(qty==true || qty=="true"){
    					   showError('拆分数量、回复数量需大于0！') ;  
       					   return false;
    				   }
    				   if(count!=assqty){
    					showError('分拆数量必须等于原数量!') ;  
    					return false;
    				   }else if(store.data.items[0].data['pd_qty']<record.data.pd_frozenqty){
    					showError('分拆后的数量不能小于已冻结数!') ;  
       					return false; 
    				   }
    				   else{
    					   var r=new Object();
    		  ...
    		  columnLines:true,
    		  store:Ext.create('Ext.data.Store',{
					fields:[{name:'pd_delivery',type:'date'},{name:'pd_qty',type:'number'},{name:'pd_deliveryreply',type:'date'},{name:'pd_qtyreply',type:'int'},
							{name:'pd_replydetail',type:'string'},{name:'pd_isok',type:'int'},{name:'pd_acceptqty',type:'int'},{name:'pd_yqty',type:'int'},
							{name:'pd_id',type:'int'},{name:'pd_sellercode',type:'string'},{name:'pd_seller',type:'string'}],
				    data:[]
    		  }),
    		  plugins: [Ext.create('Ext.grid.plugin.CellEditing', {
    		        clicksToEdit: 1,
    		  ...
    		  tbar: [{
    	            ...
    	            handler : function() {
    	                ...
    	                r.pd_sellercode=record.get('pd_sellercode');
    	                r.pd_seller=record.get('pd_seller');
    	                store.insert(store.getCount(), r);
    	            }
    	        }, {
    	            tooltip: '删除批次',
    	      ...
    	      dbfinds: [{dbGridField: "em_code", field: "pd_sellercode", trigger: null},
    	      			{dbGridField: "em_name", field: "pd_seller", trigger: null}],
    		  columns:[{
    			 dataIndex:'pd_detno',
    			 header:'序号',
    			 format:'0',
