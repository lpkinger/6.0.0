...
Ext.define("Sch.plugin.Printable", {
	...
	mainTpl: new Ext.XTemplate('{docType}<html class="' + Ext.baseCSSPrefix + 'border-box {htmlClasses}"><head><meta content="text/html; charset=UTF-8" http-equiv="Content-Type" /><title>{title}</title>{styles}</head><body class="sch-print-body {bodyClasses}"><div class="sch-print-ct {componentClasses}" style="width:100%">{[this.printLockedGrid(values)]}{[this.printNormalGrid(values)]}</div><script type="text/javascript">{setupScript}<\/script></body></html>', {
		...
		printLockedGrid: function(a) {
			...
			if (a.lockedGrid) {
				...
				h=(a.rows+1)*25+30;
				b += '<div id="lockedRowsCt" style="left:-' + a.lockedScroll + "px;margin-right:-" + a.lockedScroll + 'px;width:1100px;height:'+h+'px" class="sch-print-locked-rows-ct ' + a.innerLockedClasses + " " + Ext.baseCSSPrefix + 'grid-inner-locked">';
				b += a.lockedRows;
				b += "</div>"
			}
			return b
		...
		return {
			...
			lockedGrid: e.collapsed ? false: e,
					normalGrid: m.collapsed ? false: m,
							...
							normalWidth: b,
							rows:c.length,//zhouy 2016-12-24
	...
	setupScript: function(e, a, d, b) {
		...
		if (b) {
			...
			var f=document.getElementById("lockedRowsCt");
			f.className=f.className.replace('sch-locked-horizontal-scroll','');
			var t=f.getElementsByTagName("table")[0];
			var tbody=t.lastChild,firstRow=tbody.firstChild;
			////反馈编号：2017120063  guq		打印预览字段增加前置条件
			//反馈编号：将项目阶段/任务类型/检测条件的打印预览字段放出来
			var cells=firstRow.cells,headers=['序号','状态','编号','任务节点','前置任务','资源分配','工期','开始时间','结束时间','基线开始','基线结束','完成率','文件信息','项目阶段','计划模式','任务类型','检测条件'];
			var tr=document.createElement("tr");
			tr.className='x-grid-row x-grid-tree-node-leaf x-grid-data-row';
			tr.style.cssText='height:30px!important';
			for (var i=0;i<cells.length;i++){
				   var td = document.createElement("td");  
				   td.className='x-grid-cell x-grid-td';
				   td.style.cssText="text-align:center;line-height: 30px;background-color:#EEEEEE!important";
			       var tdContent = document.createTextNode(headers[i]);  
...
Ext.define("Gnt.locale.En", {
	...
	l10n: {
		...
		"Gnt.widget.taskeditor.TaskForm": {
			...
			baselinePercentDoneText: "基线完成",
			effortText: "工作量",
			invalidEffortText: "非法数据",
			calendarText: "日历",
			schedulingModeText: "计划模式"
		},
		"Gnt.widget.DependencyGrid": {
			idText: "ID",
			taskText: "任务名称",
			blankTaskText: "请选择任务",
			invalidDependencyText: "非法依赖关系",
			parentChildDependencyText: "父子依赖关系",
			duplicatingDependencyText: "复制依赖关系",
			transitiveDependencyText: "传递依赖关系",
			cyclicDependencyText: "循环依赖关系",
			typeText: "类型",
			lagText: "延隔时间",
			clsText: "CSS class",
			endToStartText: "FS",
...
Ext.define("Gnt.model.Assignment", {
	...
	resourceRoleField: "ResourceRole", //2017-04-19 luhg
	taskIdField: "TaskId",
	unitsField: "Units",
	...
	setResourceRole: function(a) {
		this.set(this.resourceRoleField, a)
	},
...
Ext.define("Gnt.model.Resource", {
	...
	resourceRoleField: "EmRole", //2017-04-19 luhg
	normalized: false,
	...
	getResourceRole: function() { //2017-04-19 luhg
		return this.get(this.resourceRoleField);
	},
...
Ext.define("Gnt.model.task.More", {
	...
	isAbove: function(a) {
       ...
       //while (a.parentNode !== c.parentNode) { BUG修复 zhouy 2016-12-23
		while (c.parentNode !=null && a.parentNode !== c.parentNode) {
			a = a.parentNode;
			c = c.parentNode
		}
...
Ext.define("Gnt.data.TaskStore", {
	...
	casecadeChanges:true, //zhouy  taskStore 默认迭代修改
	batchSync: true,
...
Ext.define("Gnt.view.Gantt", {
	...
	configureLabels: function() {
		var a = {
				...
				dataIndex: undefined,
				//zhouy 2016-12-13 修复添加默认editor类型
				editor:'textfield'
		};
...
Ext.define("Gnt.panel.Gantt", {
	...
	addRowOnTab: true,
	recalculateParents: true,
	cascadeChanges: true, //false  zhouy 默认不迭代修改，调整为自动迭代修改
	showTodayLine: false,
	...
	reorderTaskFakeIds : function() {
		...
		function setFakeId(rec) {
			...
			rec.set('fakeId', idx++);
			//zhouy 新增node设置虚拟序号默认不commit
			if(rec.get('Id')) rec.commit();
...
Ext.define("Gnt.field.Duration", {
	...
	setValue: function(a) {
		...
		this.callParent([b]);
		if (this.instantUpdate && !this.getSuppressTaskUpdate() && this.task) {
			//zhouy 2016-11-18 taskupdateed会执行applychanges();取消重复执行
			//this.applyChanges();
			this.task.fireEvent("taskupdated", this.task, this)
		}
	},
...
//2017-04-19 luhg
Ext.define("Gnt.column.ResourceRole", {
	extend: "Ext.grid.column.Column",
	alias: "widget.resourcerolecolumn",
	mixins: ["Gnt.mixin.Localizable"],
...
Ext.define("Gnt.widget.AssignmentGrid", {
	...
	requires: ["Gnt.model.Resource", "Gnt.model.Assignment", "Gnt.column.ResourceName","Gnt.column.ResourceRole","Gnt.column.AssignmentUnits", "Ext.grid.plugin.CellEditing"],
	assignmentStore: null,
	resourceStore: null,
	readOnly: false,
	...
	constructor: function(a) {
		this.store = Ext.create("Ext.data.JsonStore", {
			model: Ext.define("Gnt.model.AssignmentEditing", {
				...
				fields: ["ResourceName","ResourceRole"] //ResourceRole 2017-04-19 luhg
			})
		});
	...
	loadResources: function() {
		...
		for (var c = 0, a = b.getCount(); c < a; c++) {
			...
			d.push({
				...
				ResourceRole: b.getById(e).getResourceRole(),
				Units: ""
			})
		}
	...
	buildPlugins: function() {
		...
		//2018-03-08 新增筛选头 guq
		return [a,Ext.create("Ext.ux.grid.GridHeaderFilters")]
	},
	hide: function() {
	...
	buildColumns: function() {
		return [{
			...
			xtype: "resourcerolecolumn", //2017-04-19 luhg
		},
		{
...
Ext.define("Gnt.field.Assignment", {
	...
	buildButtons: function() {
		return ["->", {
			...
			handler: function() {
				...
				this.fireEvent("blur", this) //2017-02-20 luhg
			},
			scope: this
		},
		{
	...
	checkAssignments:function(r){
		var units=0,bool=true;
		Ext.Array.each(r.items,function(item){
			units+=item.get('Units');
		});
		units=units.toFixed(0); 
		if(this.task.get('PercentDone')==100){
			this.collapse();
			bool=false;
			alert('当前任务节点完成率为100%，不允许修改资源！');  
			return bool;
		}
		if(units!=100){
			bool=false;
			alert('资源分配比例之和不等于100%!'); 
			return bool;
		}
		return bool;
	},
...
Ext.define("Gnt.column.PercentDone", {
	...
	format: "0",
	align: "center",
	editor:false,//反馈编号：2017050640  guq		甘特图完成率不可编辑	 
/*	field: {
		xtype: "numberfield",
		minValue: 0,
		maxValue: 100
...
Ext.define("Gnt.widget.AssignmentEditGrid", {
	...
	requires: ["Ext.window.MessageBox", "Ext.grid.plugin.CellEditing", "Gnt.util.Data", "Gnt.model.Resource", "Gnt.model.Assignment", "Gnt.column.ResourceName","Gnt.column.ResourceRole", "Gnt.column.AssignmentUnits"],
	mixins: ["Gnt.mixin.Localizable"],
	assignmentStore: null,
	resourceStore: null,
	readOnly: false,
	...
	loadTaskAssignments: function(b) {
		...
		d = Gnt.util.Data.cloneModelSet(e,
			...
			if (h.getCount()) {
				...
				g.setResourceRole(h.getResourceRole()) //2017-04-19 luhg
			}
		});
		a.suspendEvents();
		a.loadData(d);
	...
	buildColumns: function() {
		...
		this.resourceCombo = Ext.create("Ext.form.field.ComboBox", {
			...
			listeners:{  //2017-04-19 luhg
				select:function(self,rec){
					a.editingPlugin.getActiveRecord().set(Gnt.model.Assignment.prototype.resourceRoleField,rec[0].getResourceRole());
				}
			}
		});
	...
	//判断添加资源按钮是否设为disabled 2017-01-11
	setGridDisabled:function(a){
		var bol = !a.data.leaf;
		Ext.Array.each(this.query('button'),function(item,index){
			if(item.text=='添加'){
				item.setDisabled(bol);
			}
		});
		this.plugins[0].beforeEdit = function(){
			return !bol; //非未激活不允许编辑
		};	
	}
});
Ext.define("Gnt.widget.DependencyGrid", {
	...
	//zhouy  添加虚拟序号列 2016-12-13
	fakeRender:function(d,e,b){
		var val=d||e.record.get('From');
		var c = this.dependencyStore && this.dependencyStore.getTaskStore();
		if (c && val) {
			a = c.getById(val) || c.getByInternalId(val);	
			return a.get('fakeId');
		}
		return "";
	},
	...
	//判断添加前置任务按钮是否设为disabled 2017-01-11
	setGridDisabled:function(){
		var bol = false;
		if(this.task&&this.task.data.handstatus&&this.task.data.handstatus!='未激活'){
			bol = true;		
		}
		Ext.Array.each(this.query('button'),function(item,index){
			if(item.text=='添加'){
		...
		this.getSelectionModel().setLocked(bol); //不允许选中
	}
});
//任务书附件设置 zhouy 2016-12-15
Ext.define("Gnt.widget.AttachGrid", {
	extend: "Ext.grid.Panel",
	alias: "widget.attachgrid",
	requires: ["Ext.data.JsonStore", "Ext.grid.plugin.CellEditing", "Ext.form.field.ComboBox"],
	mixins: ["Gnt.mixin.Localizable"],
	readOnly: false,
	showCls: false,
	cls: "gnt-dependencygrid",
...
Ext.define("Gnt.widget.taskeditor.TaskForm", {
	...
	showBaselineOnEditor:true,  //luhg 2017-06-28  需求编号:2017060529  注：只在任务详情显示基线，不在图中显示基线，并且隐藏基线完成率
	constructor: function(b) {
		b = b || {};
	...
	buildFields: function() {
		...
		this.items.push.call(this.items, {
			...
			this.taskNameConfig), a({
				...
				labelWidth: 90,
				width: 170,
				readOnly:true,//zhouy 2016-12-14设置任务详情中工期不可编辑
		...
		a({
			...
			width: 200,
			allowBlank: false,
			readOnly:true,//zhouy 2016-12-14设置任务详情中完成率不可编辑
		...
		this.percentDoneConfig),{
			...
			items: [{boxLabel: '自动计划', inputValue: 0},
			        {boxLabel: '手动计划', inputValue: 1}]
		...
		a({
			...
			margins: "0 0 0 6",
			allowBlank: true,
			readOnly:true,//zhouy 2016-12-14设置任务详情中工作量不可编辑
		    value: g(e.effortField)
		},
		this.effortConfig));
		if (this.showBaseline||this.showBaselineOnEditor) { //luhg 2017-06-28  需求编号:2017060529
			this.items.push.call(this.items, {
				xtype: "fieldcontainer",
			...
			a({
				...
				readOnly: !this.editBaseline,
				hidden:this.showBaselineOnEditor //luhg 2017-06-28  需求编号:2017060529
			},
		...
		Ext.iterate(b.getData(),function(e, g) {
			...
			if (f) {
				if (f.setTask) {
					...
					f.setValue(f.name=='ManuallyScheduled'?(g?1:0):g);
				}
		...
		if (this.showAdvancedForm) {
			...
			this.advancedForm = Ext.create(this.advancedFormClass || "Gnt.widget.taskeditor.TaskForm", Ext.applyIf(this.advancedFormConfig, {
				...
				},
				{
					...
					id:'phasename',
					editable:false,
					value: this.task && this.task['phasename'],
					dbBaseCondition:"pp_prjid="+prjplanid
				},{//guq 在任务详情高级中新增检测条件字段  2017-10-30  反馈编号:2017100169
					xtype:'dbfindtrigger',
					fieldLabel:'检测条件',
					name:"preconditionname",
					id:'preconditionname',
					editable:false,
					value:this.task && this.task['preconditionname'],
					dbBaseCondition:"tt_checked=1"
				},{
					xtype:'displayfield',
					name:"preconditioncode",
					id:'preconditioncode',
					hidden:true,
				},{
					xtype:'dbfindtrigger',
					fieldLabel:'后置条件',
					name:"backconditionname",
					id:'backconditionname',
					editable:false,
					value: this.task && this.task['backconditionname'],
					dbBaseCondition:"tt_type='后置条件' and tt_checked=1",
					hidden:true,
				},{
					xtype:'displayfield',
					name:"backconditioncode",
					id:'backconditioncode',
					hidden:true,
				},{
					xtype:'hidden',
					fieldLabel:'项目阶段ID',
					name:"phaseid",
					id:'phaseid',
					value: this.task && this.task['phaseid']
				}],
				task: this.task,
				taskStore: this.taskStore
			}));
...
Ext.define("Gnt.plugin.TaskEditor", {
	...
	constructor: function(a) {
		...
		if (!a.buttons) {
			this.buttons = [{
				...
				scope: this,
				//guq 当任务完结时 确认按钮为隐藏  2017-08-03  反馈编号:2017070558
				listeners:{
					afterrender:function(btn){
						var grid=Ext.getCmp("gantt_panel");
	...
	showTask: function(a) {
		if (this.taskEditor && a) {
			...
			this.show();
			this.taskEditor.dependencyGrid.setGridDisabled();
			this.taskEditor.assignmentGrid.setGridDisabled(a);
			this.setReadOnly(a.get('PercentDone')==100);
		}
	},
	//zhouy 任务完成率100% 按钮readOnly
	setReadOnly:function(bool){
		this.dockedItems.items[1].setDisabled(bool);
	},
	validate: function() {
		if (this.taskEditor) {
	...
	//判断分配比例是否等于100% 2017-01-12
	checkAssignments:function(){
		var items = this.taskEditor.assignmentGrid.store.data.items;
		if(items.length>0){
			var units=0;
			Ext.Array.each(this.taskEditor.assignmentGrid.store.data.items,function(item){
				units+=item.get('Units');
			});
			units=units.toFixed(0); 
			if(units!=100){
