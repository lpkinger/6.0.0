...
.x-module-delete-icon{
	display: none;
	opacity: .5;
	background-image: url(<%=basePath %>resource/images/icon/delete.png);
    width: 16px;
    height: 100%;
    background-repeat: no-repeat;
    line-height: 100%;
...
Ext.application({
    ...
    launch: function() {
    	Ext.create('erp.view.common.commonModule.Viewport');//创建视图
    }
});
var sob = '<%=((Employee)session.getAttribute("employee")).getCurrentMaster().getMa_user()%>';
var sobText = '<%=((Employee)session.getAttribute("employee")).getCurrentMaster().getMa_function()%>';
