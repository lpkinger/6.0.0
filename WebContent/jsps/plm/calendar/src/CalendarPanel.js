               ...
               var add=todo.add,
                ...
                var deleteData=new Array();
                 if (add.length > 0) {
                            ...
                            var data = add[i].data;
                            addData[index++] = Ext.JSON.encode(data);
                            }
                   }
                   if (update.length > 0) {
                            var index = 0;
                            for(var i=0;i<update.length;i++){
                            var data = update[i].data;
                            updateData[index++] = Ext.JSON.encode(data);
                            }
                   ...
                   if (destroy.length > 0) {
                            ...
                            var data = destroy[i].data;
                            deleteData[index++] = Ext.JSON.encode(data);
                            }
                       
                   }
                    Ext.Ajax.request({
		   		            url : basePath + "plm/calendar/save.action",
					        params:{
						       addData: unescape(addData.toString().replace(/\\/g,"%")),
						       deleteData: unescape(deleteData.toString().replace(/\\/g,"%")),
						       updateData: unescape(updateData.toString().replace(/\\/g,"%"))
						    },
		   		            method : 'post',
