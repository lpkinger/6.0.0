define([ 'app/app','common/services','service/Purc','service/SupportServices' ], function(app) {
	app.register.controller('OutFinishCtrl',['$scope', '$http', '$rootScope', '$location', '$filter','$stateParams','$modal','PurcOrder','toaster', 'OutOper', 'SupportUtil','Ring', 'Online','ngTableParams', 'SessionService','$document', 'smartySuggestor', '$window', function($scope, $http, $rootScope, $location, $filter,$stateParams,$modal,PurcOrder,toaster, 
			OutOper, SupportUtil,Ring, Online,ngTableParams,SessionService,$document, smartySuggestor, $window){
		$scope.orders = $rootScope.outFinOrders || [];
		$rootScope.title = '';
		...
		var getProdcode =  function(code,whcode,pi_id) {
		    OutOper.get({inoutNo: code,whcode:whcode,pi_id:pi_id}, function(data) {//获取成功
			  if(data.target && data.totalCount > 0){
				...
				$rootScope.outFinOrders = $scope.orders;
		...
		$scope.operate = function(order) {//跳转至对应的单据操作页面
			if( !$scope.again){
				...
				$location.path('outFinishOperation/' + order.PI_INOUTNO);	
			}
			  $scope.again = false;
		};
