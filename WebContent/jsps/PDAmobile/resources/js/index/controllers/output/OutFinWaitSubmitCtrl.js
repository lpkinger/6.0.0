define([ 'app/app','service/Purc','service/SupportServices' ], function(app) {
	app.register.controller('OutFinWaitSubmitCtrl',['$scope', '$rootScope','$stateParams','$filter', 'ngTableParams','Online','SupportUtil','OutOper','toaster','Ring', function($scope, $rootScope,$stateParams,$filter, ngTableParams,Online,SupportUtil,OutOper,toaster,Ring){
		...
		if($scope.grid){
		 ...
		 $scope.deleteAll = function (){//全部删除
		 	if(confirm("确定删除全部？")){
		 		 angular.forEach($rootScope.outFinOrders, function(value, key) {//反写剩余数
