	...
	views:[
	       ...
	       openUrl: function(record) {
	    	   ...
	    	   if(!panel){ 
	    		   ...
	    		   //ERP访问时设置显示按钮  hey
	    		   if(getUrlParam('_visit')){
	    		   	   myurl +='&_nobutton=1&_noEnableTools=1&_noBbar=1&_noc=1';
	    		   }
	    		   if( newmaster ){
	       ...
	       showCustomizeWin:function(){
	    	   ...
	    	   if(!win){
	    		   ...
	    		   Ext.Array.each(grid.columns,function(item){
	    			   if(item.text && item.text.indexOf('&#160')<0 && item.xtype!='newrownumberer'){
