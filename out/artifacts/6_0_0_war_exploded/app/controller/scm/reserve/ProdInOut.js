...
Ext.define('erp.controller.scm.reserve.ProdInOut', {
    ...
    views: [
        ...
        'core.button.Save', 'core.button.Add', 'core.button.Submit', 'core.button.Print', 'core.button.PrintA4', 'core.button.Upload', 'core.button.ResAudit','core.button.SendEdi',
        ...
        'core.button.TurnExOut', 'core.trigger.DbfindTrigger', 'core.trigger.MultiDbfindTrigger', 'core.trigger.TextAreaTrigger', 'core.button.TurnProdinoutReturn','core.button.Sync',
        ...
        'core.button.Split', 'core.button.TurnReturn', 'core.button.PrintBar', 'core.button.Printotherin', 'core.button.Printotherout', 'core.button.TurnProdOut', 'core.button.TurnYPOutReturn','core.button.CatchBatchByIncode',
        'core.button.TurnOtherIn', 'core.button.TurnOtherOut', 'core.button.TurnOtherPurcOut', 'core.button.TurnAppropriationOut', 'core.button.TurnCustReturnOut', 'core.button.TurnPurcOut','core.button.SaleTurnProdinout',
        ...
        'core.button.PrintAll', 'core.form.CheckBoxGroup', 'core.button.PrintByCondition', 'core.button.CreateBill', 'core.button.TurnGoodsOut', 'core.button.PrintMT', 'core.button.CatchBatchByOrder', 'core.button.CopyAll',
        'core.button.CatchBatchByClient', 'core.button.CatchBatchByCust','core.button.TurnPaIn','core.button.Modify', 'core.button.CreateOtherBill','core.button.CopyByConfigs','core.button.ModifyDetail','core.button.ConfirmIn',
        'core.button.CatchBatchBySacode','core.button.TurnOtherProdIO','core.button.TurnSale','core.button.CancelEdi','core.button.CatchBatchBySeller','core.button.ConfirmProdInB2c','core.button.CallProcedureByConfig'
    ],
    init: function() {
        var me = this;
        ...
        this.control({
        	...
        	'erpConfirmProdInB2cButton':{
        		click: function(btn){
        			console.log("sss");
		    		var pi_id = Ext.getCmp('pi_id').value;
    				me.FormUtil.setLoading(true);
        			Ext.Ajax.request({
        				url: basePath + 'b2c/buyer/confirmProdInB2c.action',
        				params: {
        					pi_id: pi_id
        				},
        				timeout: 600000,
        	...
        	'upexcel':{//其它采购出库单不允许导入
        		afterrender:function(btn){
        			if(caller=='ProdInOut!OtherPurcOut'){
        				btn.hide();
        			}
        		}
        	},
        	...
        	'erpSyncButton':{
        	afterrender:function(btn){
        			var pi_status = Ext.getCmp("pi_statuscode");
        			if(pi_status&&pi_status.value!='POSTED'){
        				btn.hide();
        			}
        		}
        	},
        	'erpFeeShareButton':{
    			...
    			click: function(btn){
    				Ext.Ajax.request({
                        url: basePath + 'scm/reserve/feeShare.action',
                        params: {
                            caller: caller,
                            ...
                            if (res) {
                                if (res.exceptionInfo) {
                                    ...
                                    if (res.log == null || res.log == ''){
                                    	showMessage('提示', '费用分摊成功');
                                    	window.location.reload();
                                    }
                                }
                            } else {
                                showMessage('提示', '费用分摊失败');
                            }
                        }
                    });
    			}
            ...
            'erpCreateBillButton': {
                afterrender: function(btn) {               	
                    ...
                    if (caller == 'ProdInOut!PurcCheckin' || caller == 'ProdInOut!PurcCheckout' || caller == 'ProdInOut!OutesideCheckReturn' || caller == 'ProdInOut!OutsideCheckIn' || caller == 'ProdInOut!GoodsOut' || caller == 'ProdInOut!GoodsIn' || caller == 'ProdInOut!ProcessIn') {
            ...
            'erpGridPanel2': {
                afterrender: function(grid) {
                    ...
                    if(caller == 'ProdInOut!GoodsPicking'){
                    	if (Ext.getCmp('pi_sourcetype') && Ext.getCmp('pi_sourcetype').value == "完工入库单") {
                            grid.readOnly = true;
                        }
                    }
        			...
        			console.log(grid.store.first());
        			pu_orderrype = ' ',
        			pi_class = Ext.getCmp('pi_class');
        			if(!me.GridUtil.isBlank(grid,items[0].data)) {
	    				pu_orderrype = items[0].data['pu_ordertype'];
	    			}
        			if(caller!='ProdInOut!PurcCheckin'||(status && status.value != 'POSTED' )||(pu_orderrype&&pu_orderrype!='B2C')||(pi_class&&pi_class=='采购验收单')){
        				btn.hide();
        			}
		        },
                    ...
                    if (record.data.pd_id != 0 && record.data.pd_id != null && record.data.pd_id != '') {
                      ...
                      //设置载入配件按钮
                    	...
                    	if(status && status.value == 'ENTERING'){      		
    					    btn = Ext.getCmp("erpLoadFittingButton");
    					    btn && btn.setDisabled(false); 
    					}
                    }      
                    if (!bool)
    		 ...
    		 * 
    		 */
    		'#erpLoadFittingButton': {
    			click: function(btn) {
    				//新产生的配件明细，存下原始订单序号到PD_LOADFITTING。
    				//载入之前判断此序号是否已经存在PD_LOADFITTING相等的明细行，如果有就不让再载入
    				var record = btn.ownerCt.ownerCt.getSelectionModel().getLastSelected();    				    	
    			    var pr_code = record.data.pd_prodcode;
    			    var detno = record.data.pd_pdno;
    			    var pi_id = Ext.getCmp('pi_id').value;   			    
    			    var pd_outqty = record.data.pd_outqty;
    			    me.loadFitting (pr_code,pd_outqty,pi_id,detno);
    			}
    		},
            /**
    		 * 明细分拆
           ...
           /* 'erpDeleteDetailButton': {
            ...
            'erpBarcodeButton': {
                click: function(btn) {
                	...
                	var grid = Ext.getCmp("grid");
                    var id = Ext.getCmp("pi_id").value;
                    var inoutNo=Ext.getCmp("pi_inoutno").value;
                    var pi_class=Ext.getCmp("pi_class").value;
                    var status = Ext.getCmp("pi_pdastatus").value;
                    var formCondition1 = "pi_idIS" + id +"and pi_inoutNoIS '"+inoutNo+"'"+"and pi_classIS'"+pi_class+"'";
                    var gridCondition1 = "bi_piidIS" + id +" order by bi_pdno asc,bi_barcode asc";
                    var linkCaller = '';
                   ...
                   Ext.Ajax.request({
                        ...
                        callback: function(options, success, response) {
                            ...
                            if (res.exceptionInfo) {
                                ...
                                if(linkCaller == "ProdInOut!BarcodeIn"){
	                                var result = 0;
	                                Ext.Ajax.request({
	                                ...
	                                if(result > 0 ){  //存在已经生成的条码明细或者出库
	                                	me.FormUtil.onAdd('addBarcode'+id, '条形码维护('+inoutNo+')', 'jsps/scm/reserve/setBarcode.jsp?_noc=1&whoami=' + linkCaller +'&key='+id+'&inoutno='+inoutNo+'&status='+status+'&formCondition=' + formCondition1 + '&gridCondition=' + gridCondition1);
		                            }else{
		                            	  formCondition1 = "pd_piidIS" + id +" and pi_inoutNoIS'"+inoutNo+"'"+"and pi_classIS'"+pi_class+"'";
		                                  gridCondition1 = "pd_piidIS" + id+" and pd_inqty-nvl(pd_barcodeinqty,0)>0 order by pd_pdno asc";
		                            	  var win = new Ext.window.Window({
		                                      id: 'win',
		                                      height: '80%',
		                                      width: '90%',
		                                      maximizable: true,
		                                      title:'<span><font color=blue>条形码维护['+pi_class+':'+inoutNo+']</font></span>',
		                                      buttonAlign: 'center',
		                                      layout: 'anchor',
		                                      closeAction:'hide',
		                                      items: [{
                                ...
                                }else if(linkCaller == "ProdInOut!BarcodeOut"){
                                	me.FormUtil.onAdd('addBarcode'+id, '条形码维护('+inoutNo+')', 'jsps/scm/reserve/setBarcode.jsp?_noc=1&whoami=' + linkCaller +'&key='+id+'&inoutno='+inoutNo+'&status='+status+'&formCondition=' + formCondition1 + '&gridCondition=' + gridCondition1);
                                }
                            }
                        }
            ...
            'erpConfirmInButton':{
            	afterrender: function(btn) {
                    ...
                    var Instatus = Ext.getCmp('pi_pdastatus');
                    if (poststatus && poststatus.value != 'POSTED') {
                        btn.hide();
                    }
                    if(Instatus && Instatus.value == '已入库')
                    	btn.hide();
                },
                click: function(btn) {
                    Ext.Ajax.request({
                        url: basePath + 'scm/reserve/confirmIn.action',
                        params: {
                            caller: caller,
                            id: Ext.getCmp('pi_id').value
                        },
            ...
            'erpGridWinButton': {
                ...
                beforesave: function(btn) {
                    var me = this,
                    	f = btn.ownerCt.ownerCt,
                        p = f.down('field[name=pi_statuscode]');
                    var pi_inoutno = f.down('field[name=pi_inoutno]').getValue();
                    var pi_class = f.down('field[name=pi_class]').getValue();
                    var pi_id = f.down('field[name=pi_id]').getValue();
                    var flag=false;
                    if (p && 'POSTED' == p.getValue()) {
                    	me.BaseUtil.getSetting(caller, 'allowEditFareDetail',function(v) {
                    		if(v){
                    			flag = true;
                    		}
    	        			if(flag){
    	        				f.setLoading(true);
    	        				Ext.Ajax.request({
    	        					url : basePath + 'scm/reserve/checkStatus.action',
    	        					params: {
    	        						pi_id:pi_id,
    	        					...
    	        					callback : function(options,success,response){
    	        						...
    	        						if(localJson.exceptionInfo){
    	        							flag=false;
    	        							showError(localJson.exceptionInfo);return false;
    	        						}
    	        						if(localJson.success){//没有其它应收单
    	        							flag=true;
    	        							return true;
    	        						}
    	        					}
    	        				...
    	        				flag=false;
    	        				showError("该单据已经过账,不能修改费用明细！");
    	        				return false;
    	        			}
                    	},false);
                    }
                    if(p&&'POSTED'==p.getValue()&&!flag){
                    	return false;
                    }
                    return true;
                }
            ...
            'erpDeleteButton': {
                afterrender: function(btn) {
                    var status = Ext.getCmp(me.getForm(btn).statuscodeField),
                        poststatus = Ext.getCmp('pi_statuscode'),
                        pi_ordertype = Ext.getCmp('pi_ordertype');
                    ...
                    if (pi_ordertype &&pi_ordertype.value == 'B2C') { 
						btn.hide();
					} 
                },
                click: {
                	...
                	fn: function(btn) {
                		var me = this;
                		var data = me.getCancelConfig();
                		if(data != null && data==1){
                			var win = Ext.getCmp('win');
                			if(!win) {
                				var f = Ext.getCmp('pi_cancelreason'),
                					val = f ? f.value : ''; 
                				win = Ext.create('Ext.Window', {
                					...
                					items: [{
                						margin: '10 0 0 0',
                						xtype: 'textareafield',
                						name:'pi_cancelreason',
                						height: 90,
                						width: 350,
                						fieldLabel: '<font color="red">作废原因</font>', 
                						readOnly:false,
                						allowBlank: false,
                						value: val
                					}],
                					...
                					buttons: [{
                						...
                						handler: function(btn) {
                							var reason=btn.ownerCt.ownerCt.down('textfield[name=pi_cancelreason]').value;
                		 					if (reason==null || reason==''){
                		 						showError('作废原因必须填写。');
                		 						return;
                		 					}else{
                		 						me.updateCancelReason(Ext.getCmp('pi_id').value, reason);
	                    ...
	                    if (caller == 'ProdInOut!SaleAppropriationOut' || 'ProdInOut!OtherOut' == caller) {
	                        Ext.Array.each(items, function(item) {
	                            if (!Ext.isEmpty(item.data['pd_plancode'])) {
	                            	if (item.data['pd_sfdqty'] && item.data['pd_sfdqty'] != 0) {
            ...
            'erpResPostButton': {
                ...
                click: {
                	lock: 2000,
                	fn: function(btn) {
                		me.beforeResPost(Ext.getCmp('pi_id').value);
                     }
                }
            },
            'erpTurnProdinoutIn': {
            ...
            'erpCatchBatchByIncodeButton': {
                ...
                click: function(btn) {
                	var grid = Ext.getCmp('grid');
                    Ext.Ajax.request({
                        url: basePath + 'scm/reserve/catchBatchByIncode.action',
                        params: {
                            pi_id: Ext.getCmp('pi_id').value,
                            caller: caller
                        },
                            ...
                            if (res) {
                                if (res.exceptionInfo) {
                                    ...
                                    if (res.log == null || res.log == ''){
                                    	...
                                    	showMessage('提示', '按入仓单抓取成功');
                                    }
                                }
                            } else {
                                showMessage('提示', '按入仓单抓取失败');
                            }
                        }
                    });
                }
            ...
            'erpCatchBatchBySellerButton':{//按业务员抓取批号
                ...
                click: function(btn) {
                    Ext.Ajax.request({
                        url: basePath + 'scm/reserve/catchBatchBySeller.action',
                        params: {
                            caller: caller,
                            pi_id: Ext.getCmp('pi_id').value
                        },
                            ...
                            if (res) {
                                if (res.exceptionInfo) {
                                    ...
                                    if (res.log == null || res.log == ''){
                                    	...
                                    	showMessage('提示', '按订单抓取成功');
                                    }
                                }
                            } else {
                                showMessage('提示', '按订单抓取失败');
                            }
            ...
            'erpTurnPaInButton': {
            	afterrender: function(btn) {
                    var pi_invoicecode = Ext.getCmp('pi_invoicecode'), pi_packingcode = Ext.getCmp('pi_packingcode'),
                    	pi_invostatuscode = Ext.getCmp('pi_invostatuscode'), pi_class = Ext.getCmp('pi_class').value;
                    	pi_statuscode = Ext.getCmp('pi_statuscode');
                    if(pi_class !='销售退货单' && pi_class !='出货单'&& pi_class !='拨出单'){
                    	 btn.hide();
                    }
    			...
    			click: function(btn){
    				warnMsg("确定要转入销售单吗?", function(btn){
    					if(btn == 'yes'){
    						me.FormUtil.getActiveTab().setLoading(true);//loading...
    	    				Ext.Ajax.request({
    	    			   		url : basePath + 'scm/reserve/turnSale.action',
    	    			   		params: {
    	    			   			id: Ext.getCmp('pi_id').value,
    	    			   			caller : caller
    	    			   		},
    ...
    getCodeCondition: function() {
        ...
        switch (caller) {
            ...
            case 'ProdInOut!OutsidePicking': //委外领料单
                field = "ma_vendcode";
                tFields = 'pi_cardcode,pi_title,pi_departmentcode,pi_departmentname,pi_receivecode,pi_receivename';
                fields = 'ma_vendcode,ma_vendname,ma_departmentcode,ma_departmentname,ma_apvendcode,ma_apvendname';
                tablename = 'Make';
                myfield = 'ma_code';
                break;
            case 'ProdInOut!OutsideReturn': //委外退料单
	...
	beforeResPost : function(id){//检查是否有物料号+批号+仓库出现在已过账的未制作凭证的成本调整单中
		var me = this;
		var form = Ext.getCmp('form');
		Ext.Ajax.request({
			url : basePath +'scm/reserve/resPostCheck.action?caller='+caller,
			params: {
				id: id
			},
			method : 'post',
			callback : function(options,success,response){
				...
				if(localJson.success){
					if(localJson.log==""){
						me.FormUtil.onResPost(id);
					}else{
						warnMsg("行"+localJson.log+"物料批次有关联的成本调整单，反过账成功将删除关联的成本调整单明细，请联系财务确认是否继续?",
							function(btn){
							if(btn == 'yes'){
								me.FormUtil.onResPost(id);
							}
	...
	getCancelConfig:function(){
		var data = '';
    	Ext.Ajax.request({
	   		url : basePath + 'scm/reserve/getCancelConfig.action',
	   		async: false,
	   		params:
	   		{
	   			caller : caller
	...
	updateCancelReason: function(id, val) {
		Ext.Ajax.request({
			url: basePath + 'scm/reserve/updateCancelReason.action',
			params: {
				id: id,
				value: val,
				caller : caller
	...
	loadFitting : function (pr_code,pd_outqty,pi_id,detno){
		Ext.Ajax.request({
	   		url : basePath + 'scm/reserve/getFittingData.action',	   
	   		params: {
	   			caller: caller,
	   			pr_code: pr_code,
	   			qty:pd_outqty,
