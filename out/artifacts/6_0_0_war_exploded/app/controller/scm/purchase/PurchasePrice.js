Ext.QuickTips.init();
Ext.define('erp.controller.scm.purchase.PurchasePrice', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','scm.purchase.PurchasePrice','core.grid.Panel2','core.toolbar.Toolbar','core.button.Appstatus','core.button.ResAppstatus',
      		'core.button.Save','core.button.Add','core.button.Submit','core.button.Print','core.button.Upload','core.button.ResAudit',
				...
				'core.button.ResSubmit','core.button.Banned','core.button.ResBanned','core.button.Abate','core.button.ResAbate','core.trigger.MultiDbfindTrigger',
			'core.trigger.DbfindTrigger','core.trigger.TextAreaTrigger','core.form.YnField','core.form.FileField','core.button.Sync','core.button.SyncSpecial',
			'core.button.CopyAll','core.grid.detailAttach','core.button.PrintByCondition'],
    init:function(){
    	var me = this;
    	me.alloweditor = true;
    	this.control({
    		...
    		'erpSaveButton': {
    			click: function(btn){
    				...
    				var grid = Ext.getCmp('grid'),items = grid.store.data.items,
    					...
    					vendcode = Ext.getCmp('pp_vendcode') == null ? null : Ext.getCmp('pp_vendcode').value,
    					vendname = Ext.getCmp('pp_vendname') == null ? null : Ext.getCmp('pp_vendname').value,
    					currencys = Ext.getCmp('pp_currencys') == null ? null : Ext.getCmp('pp_currencys').value,
    					rate = Ext.getCmp('pp_rate') == null ? null : Ext.getCmp('pp_rate').value;
    				...
    				Ext.each(items, function(item){
    					if(item.dirty && item.data[grid.necessaryField] != null && item.data[grid.necessaryField] != ""){
    						if(item.data['ppd_currency'] == null || item.data['ppd_currency'] == ''){
    							if(!Ext.isEmpty(currencys)){
    								item.data['ppd_currency'] = currencys;
    							}else{
    						...
    						if(item.data['ppd_vendcode'] == null || item.data['ppd_vendcode'] == ''){
    							if(!Ext.isEmpty(vendcode)){
    								item.data['ppd_vendcode'] = vendcode;
    								item.data['ppd_vendname'] = vendname;
    							}else{
    						...
    						if((item.data['ppd_rate'] ==null || item.data['ppd_rate'] == '') && !Ext.isEmpty(rate)){
    							item.data['ppd_rate'] = rate;
    						}
    						if(!Ext.isEmpty(enddate)){
			...
			'erpBannedButton': {//禁用
				afterrender: function(btn){
					var status = Ext.getCmp('pp_statuscode');
					if(status && (status.value == 'DISABLE' || status.value == 'BANNED')){
						btn.hide();
					}
				},
    			click: function(btn){
    				this.FormUtil.onBanned(Ext.getCmp('pp_id').value);
    			}
    		},
    		'erpResBannedButton': {//反禁用
    			...
    			click: function(btn){
    				this.FormUtil.onResBanned(Ext.getCmp('pp_id').value);
    			}
    		},
    		'erpResAbateButton':{//转有效
    			...
    			click: function(btn){
	    			...
	    			Ext.Ajax.request({
	        			url : basePath + "scm/purchase/resabatepurchaseprice.action?caller=PurchasePrice",
	        			params:{
	        				id: ppdid
	        			},
	        			method:'post',
    		...
    		'erpAbateButton':{//失效
    			...
    			click: function(btn){
	    			...
	    			Ext.Ajax.request({
	        			url : basePath + "scm/purchase/abatepurchaseprice.action?caller=PurchasePrice",
	        			params:{
	        				id: ppdid
	        			},
	        			method:'post',
    		...
    		'erpResAppstatusButton':{//转未认定
    			...
    			click: function(btn){
	    			...
	    			Ext.Ajax.request({
	        			url : basePath + "scm/purchase/resAppstatuspurchaseprice.action?caller=PurchasePrice",
	        			params:{
	        				id: ppdid
	        			},
	        			method:'post',
    		...
    		'erpAppstatusButton':{//转合格
    			...
    			click: function(btn){
	    			...
	    			Ext.Ajax.request({
	        			url : basePath + "scm/purchase/appstatuspurchaseprice.action?caller=PurchasePrice",
	        			params:{
	        				id: ppdid
	        			},
	        			method:'post',
            ...
            'erpSyncSpecialButton': {
            	afterrender: function(btn) {
                    var form = btn.ownerCt.ownerCt, s = form.down('#pp_statuscode');
                    if (s.getValue() != 'AUDITED')
                        btn.hide();
                }
            },'dbfindtrigger[name=ppd_prodcode]':{
            	 focus: function(t) {
                    t.setHideTrigger(false);
                    t.setReadOnly(false);
                    t.dbBaseCondition = "nvl(pr_groupcode,' ')<>'用品'";
                    var pp_kind = Ext.getCmp("pp_kind");
                    if(pp_kind&&pp_kind.value =='用品'){
                    	t.dbBaseCondition = "nvl(pr_groupcode,' ')='用品'";
                    }
                }
            }
	...
	copy: function(){
		...
		if(v > 0) {
			...
			Ext.Ajax.request({
				url: basePath + 'scm/purchase/copyPurchasePrice.action',
				params: {
					caller : caller,
					id : v
				},
				callback: function(opt, s, r){
					...
					if(res.data) {
						var url = 'jsps/scm/purchase/purchasePrice.jsp?formCondition=pp_idIS' 
