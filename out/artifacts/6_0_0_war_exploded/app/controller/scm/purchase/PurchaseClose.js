Ext.QuickTips.init();
Ext.define('erp.controller.scm.purchase.PurchaseClose', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','scm.purchase.PurchaseClose','core.grid.Panel2','core.toolbar.Toolbar',
	...
	init:function(){
		...
		this.control({
			...
			'erpAddButton': {
				click: function(){
					me.FormUtil.onAdd('addPurchaseClose', '新增采购结案申请单', 'jsps/scm/purchase/purchaseClose.jsp');
				}
			},
			'erpCloseButton': {
				click: function(btn){
    		...
    		'dbfindtrigger[name=pcd_orderdetno]': {
    			focus: function(t){
    				t.setHideTrigger(false);
    				t.setReadOnly(false);
    				var record = Ext.getCmp('grid').selModel.lastSelected;
    				var code = record.data['pcd_ordercode'];
    				...
    				if(code == null || code == ''){
    					...
    					t.dbBaseCondition = "pu_code='" + code + "'";
    				}
    				var cond ="(pu_code='"+code+"' ";
    				var arr = new Array();
    				Ext.each(grid.store.data.items, function(item){
    					if(item.data['pcd_ordercode'] != null && item.data['pcd_ordercode'] != ''
    						&& item.data['pcd_orderdetno'] != null && item.data['pcd_orderdetno'] !=''){
    						if(item.data['pcd_ordercode'] ==code){
    							arr.push("pd_detno<>"+item.data['pcd_orderdetno']);
    						}
    					}
    				});
    				if(arr.length > 0){
    		...
    		'multidbfindtrigger[name=pcd_orderdetno]': {
    			focus: function(t){
    				t.setHideTrigger(false);
