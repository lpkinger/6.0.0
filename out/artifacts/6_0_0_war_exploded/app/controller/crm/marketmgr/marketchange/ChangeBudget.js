Ext.QuickTips.init();
Ext.define('erp.controller.crm.marketmgr.marketchange.ChangeBudget', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		'core.form.Panel','crm.marketmgr.marketchange.ChangeBudget','core.grid.Panel2','core.toolbar.Toolbar',
	...
	init:function(){
		...
		this.control({
			...
			'erpAddButton': {
				click: function(){
					me.FormUtil.onAdd('addChangeBudget', '新增预算变更单', 'jsps/crm/marketmgr/marketchange/changeBudget.jsp');
				}
			},
			'erpCloseButton': {
				click: function(btn){
    		...
    		'dbfindtrigger[name=cbd_ppddetno]': {
    			focus: function(t){
    				t.setHideTrigger(false);
    				t.setReadOnly(false);
    				var record = Ext.getCmp('grid').selModel.lastSelected;
    				var code = record.data['cbd_projcode'];
    				if(code == null || code == ''){
    					showError("请先选择要变更的项目编号!");
    					t.setHideTrigger(true);
    					t.setReadOnly(true);
    				} else {
    					t.dbBaseCondition = "pp_code='" + code + "'";
    				}
    			}
    		}
		});
