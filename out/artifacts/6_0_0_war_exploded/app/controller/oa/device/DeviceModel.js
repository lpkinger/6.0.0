Ext.QuickTips.init();
Ext.define('erp.controller.oa.device.DeviceModel', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    views:[
    		'core.form.Panel','oa.device.DeviceModel','core.grid.Panel2','core.form.MultiField','core.form.FileField',
		...
		this.control({ 
			'erpGridPanel2': { 
    			afterrender: function(grid){
    				var status = Ext.getCmp('dm_statuscode');
			...
			'erpDeleteButton' : {
    			click: function(btn){
    				me.FormUtil.onDelete(Ext.getCmp('dm_id').value);
    			}
    		},
			'erpUpdateButton': { 
				click: function(btn){
			...
			'erpAddButton': {
    			click: function(){
    				me.FormUtil.onAdd('addDeviceModel', '新增设备申购单', 'jsps/oa/device/deviceModel.jsp?whoami='+ caller);
    			}
    		},
			'erpCloseButton': {
    			click: function(btn){
			...
			'erpSubmitButton': {
				...
				click: function(btn){
					me.FormUtil.onSubmit(Ext.getCmp('dm_id').value);
				}
			},
			'erpResSubmitButton': {
				afterrender: function(btn){
				...
				click: function(btn){
					me.FormUtil.onResSubmit(Ext.getCmp('dm_id').value);
				}
			},
			'erpAuditButton': {
				afterrender: function(btn){
				...
				click: function(btn){
					me.FormUtil.onAudit(Ext.getCmp('dm_id').value);
				}
			},
			'erpResAuditButton': {
				afterrender: function(btn){
				...
				click: function(btn){
					me.FormUtil.onResAudit(Ext.getCmp('dm_id').value);
				}
			}
		});
	},
