...
Ext.define('erp.controller.pm.outsource.MakeMaterialScrap', {
	...
	init:function(){
		...
		this.control({
			...
			'checkbox[id=whcode]' : {
				afterrender : function(f) {
					me.BaseUtil.getSetting('MakeMaterial!OS!Scrap', 'GroupWarehouse.OS', function(bool) {
						f.setValue(bool);
                    });
				}
			...
			'combo[id=groupPurs]': {
				beforerender: function(f) {
					me.BaseUtil.getSetting('MakeMaterial!OS!Scrap', 'isGroupPurc', function(v) {
						if(v){
							f.show();							
						}
					});
					me.BaseUtil.getSetting('MakeMaterial!OS!Scrap', 'showUserFactoryWh', function(v) {//物料分仓库存只显示登录用户所属工厂对应仓库库存信息
						var grid = Ext.getCmp('editorColumnGridPanel');
			...
			'erpEditorColumnGridPanel':{
				afterrender : function(f) {
					me.BaseUtil.getSetting('MakeMaterial!OS!Scrap', 'Select!OS!issue', function(bool) {
						isSelect = bool;
                    });
				},
	...
	showReplace: function(condition,callback){
		...
		Ext.Ajax.request({
	   		...
	   		params: {
	   			...
	   			fields: 'mp_mmid,mp_detno,mp_thisqty,mp_canuseqty,mp_repqty,mp_haverepqty,mp_scrapqty,mp_turnscrapqty,mm_onlineqty,mm_backqty,mp_remark,mp_prodcode,pr_detail,pr_spec,pr_unit,wh_code',
	   			condition: condition 
	   		},
	   		async: false,
	   		method : 'post',
