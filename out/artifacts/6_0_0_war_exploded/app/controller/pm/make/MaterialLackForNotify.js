Ext.QuickTips.init();
Ext.define('erp.controller.pm.make.MaterialLackForNotify', {
    ...
    views:[
     		...
     		'core.grid.YnColumn','core.button.PurchaseNotifyCancel','core.button.PurchaseNotifyCancelAll',
     		'core.button.PurchaseNotifyCancelById','core.button.PurchaseNotifyCancelByCondition','core.button.PurchaseNotifyAllCancel'
     	],
    init:function(){
    	var me = this;
    	me.resized = false;
    	this.control({ 
    		'erpBatchDealFormPanel': {
    			alladded: function(form){
    				...
    				me.resize(form, grid);  
    				var thiscondition=grid.defaultCondition;
    				var n=thiscondition.indexOf("1=1", 0); 
    				if (n>=0){
    					thiscondition=thiscondition.substring(0, n-1);
    				}
    				if (thiscondition==null || thiscondition==''){
    					thiscondition+="1=1";
    				} 
    				if(caller=='PurchaseNotify!Cancel'){
    					if(Ext.getCmp('cmdstatus').value=='NOW') grid.defaultCondition=thiscondition+" and ((pr_ltinstock=-1 and (months_between(last_day(sysdate),pn_delivery)>=0 or (TO_CHAR(SYSDATE,'DD')>25 and months_between(pn_delivery,last_day(sysdate))<=1))) or (NVL(pr_ltinstock,0)>=0 and sysdate+NVL(pr_ltinstock,0)+1>pn_delivery))";
        				else if (Ext.getCmp('cmdstatus').value=='WAIT')  grid.defaultCondition=thiscondition+" and not ((pr_ltinstock=-1 and (months_between(last_day(sysdate),pn_delivery)>=0 or (TO_CHAR(SYSDATE,'DD')>25 and months_between(pn_delivery,last_day(sysdate))<=1))) or (NVL(pr_ltinstock,0)>=0 and sysdate+NVL(pr_ltinstock,0)+1>pn_delivery))";
        			}
    		...
    		'combo[name=cmdstatus]':{ 
    			change:function(field){
    				...
    				if(Ext.getCmp('cmdstatus').value=='NOW') grid.defaultCondition=thiscondition+" and ((pr_ltinstock=-1 and (months_between(last_day(sysdate),pn_delivery)>=0 or (TO_CHAR(SYSDATE,'DD')>25 and months_between(pn_delivery,last_day(sysdate))<=1))) or (NVL(pr_ltinstock,0)>=0 and sysdate+NVL(pr_ltinstock,0)+1>pn_delivery))";
    					...
    					grid.defaultCondition=thiscondition;
    				}
