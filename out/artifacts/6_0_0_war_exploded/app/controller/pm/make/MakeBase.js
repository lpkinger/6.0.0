...
Ext.define('erp.controller.pm.make.MakeBase', {
    ...
    views:[
            'core.button.PrintByCondition','core.form.Panel','pm.make.MakeBase','core.grid.Panel2','core.toolbar.Toolbar','core.form.MultiField','core.form.YnField',
    ...
    init:function(){
        ...
        this.control({
            ...
            'erpFormPanel':{
              //20180607由于grid采用同步方式，欧盛会有比较多单据明细行很多1000多行，页面渲染就会慢，同步的方式会导致gridutil 方法 getGridColumnsAndStore获取不到form，无法根据主表状态设置grid是否只读，所以在form的afterload事件设置grid的可读状态
            ...
            'erpGetOSVendorButton': { 
                ...
                click: function(btn){ 
                    // confirm box modify
                    // zhuth 2018-2-1
                    Ext.Msg.confirm('提示', '确定要自动获取委外商和单价?', function(btn) {
                        if(btn == 'yes') {
                            Ext.Ajax.request({
                ...
                click: function(btn){
                     ...
                     buttons:[{
                        ...
                        handler:function(btn){
                               ...
                               var getprice2=Ext.getCmp('radio2').value;
                               var getprice3=Ext.getCmp('radio3').value;
                               var url='';
                               if(getprice){
                               ...
                               if(getprice3){
                            	   url='/scm/purchase/getLastMakePrice.action';
                               }
                               Ext.Ajax.request({
                                url: basePath + url,
                                params: {
...
//                                  me.GridUtil.loadNewStore(grid, gridParam);  
            ...
            'erpTurnOSMakeButton':{
                ...
                click: function(btn){
                    // confirm box modify
                    // zhuth 2018-2-1
                    Ext.Msg.confirm('提示', '确定要转本单据为委外单?', function(btn) {
                        if(btn == 'yes') {
            ...
            'erpTurnOSToMakeButton':{
                ...
                click: function(btn){
                    // confirm box modify
                    // zhuth 2018-2-1
                    Ext.Msg.confirm('提示', '确定要转本单据为制造单?', function(btn) {
                        if(btn == 'yes') {
                            var kind = Ext.getCmp('ma_kind'),
                                bg = 'background:#fffac0;color:#515151;';
                            Ext.create('Ext.window.Window', {
            ...
            'field[name=ma_prodcode]': {
                change: function(f){
                    if(f.value != null && f.value != ''){
                        me.FormUtil.getFieldsValue('BOM left join customer on bo_custcode=cu_code', 'bo_id,bo_custcode,cu_name', "bo_mothercode='" + f.value + "'", 'ma_bomid,ma_custcode,ma_custname');
                    }
                }
            },
            'field[name=ma_qty]': {
