Ext.QuickTips.init();
Ext.define('erp.controller.fa.arp.ShowVmDetail', {
    extend: 'Ext.app.Controller',
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views: ['core.trigger.DbfindTrigger', 'fa.arp.vmQuery.VmDetailGrid', 'fa.arp.vmQuery.ShowVmDetail', 'core.form.MonthDateField', 
    ...
    init:function(){
    	...
    	this.control({
    		...
    		'button[name=export]': {
    			click: function() {
    				var grid = Ext.getCmp('vmdetailgrid');
    				me.BaseUtil.exportGrid(grid, '应付总账明细'+'-'+yearmonth+'-'+vendname+'-'+currency+'_','  供应商名称:'+vendname+'    币别:'+currency+'    期间:'+yearmonth);
    			}
    		},
    		'button[name=query]':{
    			click:function(btn){
    	    ...
    	    buttons: [{
	    		...
	    		handler: function(btn) {
	    			...
	    			if(showApDetailVal){
	    				...
	    				me.getConditionDetail(con,'fa/arp/VmQueryController/getVmDetailByIdDetail.action');
	    			}else{
	    				grid.columns = grid.defaultColumns;
	    				me.getConditionDetail(con,'fa/arp/VmQueryController/getVmDetailById.action');
	    			}
					//隐藏筛选框
    ...
    query: function(cond) {
    	...
    	Ext.Ajax.request({
    		url: basePath + 'fa/arp/VmQueryController/getVmDetailById.action',
    		params: {
...
//    				grid.store.loadData(res.data);
    ...
    onGridItemClick:function(selModel, record){
		...
		if(value>0){
			...
			var keyField ='';
			var caller = '';
			var pfField = '', type = record.get('tb_kind'), t = record.get('tb_table');
			...
			if(type == '应付发票'){
				...
				pfField = 'abd_abid';
				caller = 'APBill!OTDW';
			}else if(type =='应付暂估' || type =='暂估减少' || type =='暂估增加'){
			...
			} else if(type =='应付款转销' && t == 'paybalance'){
				url ='jsps/fa/arp/payBalancePRDetail.jsp?whoami=PayBalance!APRM';
				keyField = 'pb_id';
				pfField = 'pbd_pbid';
				caller = 'PayBalance!APRM';
			} else if(type =='应付款转销' && t == 'apbill'){
				url ='jsps/fa/ars/apbill.jsp?whoami=APBill!CWIM';
				keyField = 'ab_id';
				pfField = 'abd_abid';
				caller = 'APBill!CWIM';
			} else if(type =='应收冲应付' && t == 'paybalance'){
				url ='jsps/fa/arp/paybalance.jsp?whoami=PayBalance!YSCYF';
				keyField = 'pb_id';
				pfField = 'pbd_pbid';
				caller = 'PayBalance!YSCYF';
			}else if(type =='预付退款单'){
				url ='jsps/fa/arp/prepay.jsp?whoami=PrePay!Arp!PAPR';
				keyField = 'pp_id';
				pfField = 'ppd_ppid';
				caller = 'PrePay!Arp!PAPR';
			}
