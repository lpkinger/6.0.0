Ext.QuickTips.init();
Ext.define('erp.controller.fa.ars.ShowCmCopDetail', {
    extend: 'Ext.app.Controller',
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views: ['core.trigger.DbfindTrigger', 'fa.ars.cmCopQuery.CmDetailGrid', 'fa.ars.cmCopQuery.ShowCmDetail', 'core.form.MonthDateField', 
    ...
    init:function(){
    	...
    	this.control({ 
    		...
    		'button[name=export]': {
    			click: function() {
    				var grid = Ext.getCmp('cmcopdetailgrid');
    				me.BaseUtil.exportGrid(grid, '应收总账明细'+'-'+yearmonth+'-'+custname+'-'+currency+'_'+cop,'  公司:'+cop+'  客户名称:'+custname+'    币别:'+currency+'    期间:'+yearmonth);
    			}
    		},
    		'button[name=query]':{
    			click:function(btn){
    	    ...
    	    buttons: [{
	    		...
	    		handler: function(btn) {
	    			...
	    			if(showArDetailVal){
	    				...
	    				me.getConditionDetail(con,'fa/ars/CmQueryController/getCmCopDetailByIdDetail.action');
	    			}else{
	    				grid.columns = grid.defaultColumns;
	    				me.getConditionDetail(con,'fa/ars/CmQueryController/getCmCopDetailById.action');
	    			}
					//隐藏筛选框
    ...
    query: function(cond) {
    	...
    	Ext.Ajax.request({
    		url: basePath + 'fa/ars/CmQueryController/getCmCopDetailById.action',
    		params: {
