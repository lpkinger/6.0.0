Ext.QuickTips.init();
Ext.define('erp.controller.fa.ars.SaleKind', {
    extend: 'Ext.app.Controller',
    FormUtil: Ext.create('erp.util.FormUtil'),
    GridUtil: Ext.create('erp.util.GridUtil'),
    BaseUtil: Ext.create('erp.util.BaseUtil'),
    views:[
      		/*'core.form.Panel','fa.ars.SaleKind','core.grid.Panel2','core.toolbar.Toolbar','core.toolbar.Toolbar2',*/
      		'core.grid.Panel4','core.toolbar.Toolbar3',
      		'core.button.Save','core.button.Add','core.button.Submit','core.button.Print','core.button.Upload','core.button.ResAudit',
      			'core.button.Audit','core.button.Close','core.button.Delete','core.button.Update','core.button.DeleteDetail','core.button.ResSubmit',
      		'core.trigger.DbfindTrigger','core.trigger.TextAreaTrigger','core.form.YnField','core.trigger.CateTreeDbfindTrigger'
      	],
    init:function(){
    	var me = this;
    	...
    	this.control({
    		...
    		'cateTreeDbfindTrigger[name=sk_salecatename]': {
    			focus: function(t){
    				Ext.getCmp('grid').dbfinds = [{
    					dbGridField:'ca_code', 
    					field: 'sk_salecatecode'
    				},{
    					dbGridField:'ca_name', 
    					field: 'sk_salecatename'
    				}];
    			}
    		},
    		'cateTreeDbfindTrigger[name=sk_costcatename]': {
    			focus: function(t){
    				Ext.getCmp('grid').dbfinds = [{
    					dbGridField: 'ca_code', 
    					field: 'sk_costcatecode'
    				},{
    					dbGridField: 'ca_name', 
    					field: 'sk_costcatename'
    				}];
    			}
    		}
    	});
	...
	beforeSave: function(){
		...
		Ext.each(items,function(item,index){
			Ext.each(grid.columns, function(c){
				if(item.data[c.dataIndex]!=item.raw[c.dataIndex]&&item.data[c.dataIndex]!=0)
					{
			...
			if(rowNo.length==0){
				...
				if(result){
					...
					var j= jsonGridData.length = rowNo.length;
					for(var m=0;m<j;m++){
						var x=rowNo[m]-1;
						var data = s[x].data;
	...
	*/},
	...
	update:function(param){
		console.log(param);
		Ext.Ajax.request({
		    url: basePath+'fa/ars/updateSaleKind.action',
		    params:{
		    	param:param
		    },
