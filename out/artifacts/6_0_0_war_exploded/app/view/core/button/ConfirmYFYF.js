 ...
 * 预付-应付明细确认
 */	
Ext.define('erp.view.core.button.ConfirmYFYF',{ 
		extend: 'Ext.Button', 
		alias: 'widget.erpConfirmYFYFButton',
		//iconCls: 'x-button-icon-submit',
    	cls: 'x-btn-gray',
    	id:'confirmyfyf',
    	text: $I18N.common.button.erpConfirmYFYFButton,
    	style: {
    		marginLeft: '10px'
        },
        width: 140,
		...
		listeners: {
	        click: function(m){
	        	...
	        	if(record != null){
					Ext.getCmp('confirmyfyf').turn(record);
				} else {
		...
		turn: function(record){
			...
			me.BaseUtil.getSetting('sys', 'useBillOutAP', function(editable) {
				win = new Ext.window.Window({
				    ...
				    items: [{
			    		...
			    		items:[{
			    			fieldLabel:'供应商号',
			    			value:record.data.vm_vendcode,
			    			readOnly: true,
			    			fieldStyle:'background:#f0f0f0;border: 0px solid #8B8970;color:blue;',
			    			id:'vecode'
			    		},{
			    			fieldLabel:'供应商名'	,
			    			value:record.data.ve_name,
			    			fieldStyle:'background:#f0f0f0;border: 0px solid #8B8970;color:blue;',
			    			readOnly: true
			    		},{
			    			fieldLabel:'币别',
			    			value:record.data.vm_currency,
			    			fieldStyle:'background:#f0f0f0;border: 0px solid #8B8970;color:blue;',
			    			readOnly: true,
			    			id:'currency'
			    		},{
			    		    ...
			    		    value:record.data.vm_prepaybalance,
			    		    readOnly: false,
			    		    id:'vm_prepaybalance'
			    		},{
			    		    fieldLabel:'预付金额',
						...
						selModel: Ext.create('Ext.selection.CheckboxModel',{
							...
							countAmount: function(record){
				            	...
				            	Ext.each(items,function(item,index){
			                		taxsum = taxsum + Number(item.data['PP_THISAMOUNT']);
			                	});
				               	Ext.getCmp('ppamount').setValue(taxsum.toFixed(2));
					        }
						}),
						columns : [ {
							text : '预付单号',
						...
						store : new Ext.data.Store({
							fields : [ 'PP_CODE', 'PP_DATE', 'PP_JSAMOUNT',
									'PP_HAVEBALANCE', 'PP_THISAMOUNT','PP_TYPE',
									'PP_ID','PP_VENDCODE','PP_CURRENCY', 'PP_ORDERCODE' ],
							proxy : {
								type : 'ajax',
								url : basePath + 'fa/arp/getPrePay.action',
								reader : {
									type : 'json',
									root : 'data'
								}
						...
						store : new Ext.data.Store({
							fields : [ 'AB_CODE', 'AB_DATE', 'AB_APAMOUNT',
									'AB_PAYAMOUNT', 'AB_ORDERCODE','AB_CLASS',
									'AB_ID','AB_VENDCODE','AB_CURRENCY', 
									'AB_THISAMOUNT', 'AB_INVOAMOUNT', 'AB_INVOSTATUS' ],
							proxy : {
								type : 'ajax',
								url : basePath + 'fa/arp/getAPBill.action',
								reader : {
									type : 'json',
									root : 'data'
								}
				    	...
				    	listeners: {
   				    		...
   				    		click: function(btn) {
   		        				...
   		        				var data1 = Ext.Array.map(items1, function(item){
   		        					return {pp_id: item.get('PP_ID'), pp_thisamount: item.get('PP_THISAMOUNT')}; 
   		        				...
   		        				if(data1.length > 0 || data2.length > 0) {
   		        					Ext.Ajax.request({
   		        				   		url : basePath + 'fa/confirmPrePayAPBill.action',
   		        				   		params: {
   		        				   			vmid: Ext.getCmp("vm_id").value,
   		        				   			thisamount: Ext.getCmp("vm_prepaybalance").value,
   		        				   		...
   		        				   		callback : function(options,success,response){
   		        			    			...
   		        			    			if(localJson.success){
   		        			    				...
   		        			    				record.set('vm_prepaybalance', Ext.getCmp('vm_prepaybalance').value);
				...
				win.down('#grid1').getStore().load({
					params: {
						vendcode: record.get('vm_vendcode'),
						currency: record.get('vm_currency')
					}
				});
