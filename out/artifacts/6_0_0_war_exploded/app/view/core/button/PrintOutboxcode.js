 ...
 * 多种打印格式，按箱号打印
 */	
Ext.define('erp.view.core.button.PrintOutboxcode',{ 
		id:'printOutboxcode',
		extend: 'Ext.Button', 
		alias: 'widget.erpPrintOutboxcodeButton',
        ...
        handler: function(){
        	...
        	var lps_Barcaller = '';
        	if(caller == 'profit'){
				lps_Barcaller = 'BarStock!OutBoxPrint';
			}
        	var me = this;
        	var bool = false;
        	var bool2 = false;
 	      /*  var items = grid.store.data.items;*/
        	var items = grid.selModel.getSelection();
 	        Ext.each(items, function(item, index){
 	        	if(this.data['bdd_outboxcode'] != null && this.data['bdd_outboxcode'] != ''
 	        		&& this.data['bdd_outboxcode'] != '0' && this.data['bdd_outboxcode'] != 0){
 	        		bool2 = true;  
	    	...
	    	if(type =='ALL'){//全部打印
		    	...
		    	var data = new Array();
	 	        Ext.each(items, function(item, index){
	 	        	if(this.data['bdd_outboxcode'] != null && this.data['bdd_outboxcode'] != ''
	 	        		&& this.data['bdd_outboxcode'] != '0' && this.data['bdd_outboxcode'] != 0){
	 	        	     o = this.data['bdd_outboxcode'];   		
	 	        	}
	 	        	data.push(o);
	 	        });
	 	        params['data'] = data.join(",");	 	       
	 	        condition='( bdd_bsid='+ key +' and bdd_outboxcode in ('+data+'))';
	    	}	    
	       Ext.Ajax.request({
						...
						buttons: [{
							...
							handler: function(b) {
								...
								if(printers && printers.value!= null){
									me.zebraPrint("OutboxInPrint",printers.value,dpi.value,type);
									b.ownerCt.ownerCt.close();
								}else{
									alert("请选择打印机 ");
								}   
   	...
   	zebraPrint:function(caller,printer,dpi,type){
	    	...
	    	if(type =='ALL'){//全部打印
		    	...
		    	var data = new Array();
	 	        ...
	 	        condition='(bi_piid='+ key +' and bi_id in ('+data.join(",")+'))';
	    	}	
