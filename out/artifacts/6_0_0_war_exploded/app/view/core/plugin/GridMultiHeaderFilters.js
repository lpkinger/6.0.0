	Ext.define('erp.view.core.plugin.GridMultiHeaderFilters',{
        ...
        for(var c=0; c < columns.length; c++)
        {
            ...
            if(column.filter)
            {
                var filterContainerConfig = {
                    ...
                    listeners: {
                        ...
                        mousedown: function(e)
                        {
                        	...
                        	var mouseClickX = e.browserEvent.clientX;
                        	var width = e.currentTarget.style.width.split('px')[0];
                        	var column = Ext.getCmp(e.currentTarget.parentNode.getAttribute('id'));
                        	if(clientWidth-mouseClickX<=width&&!column.firstFocus){
                        		column.firstFocus = true;
                        		var scrollY = Ext.ComponentQuery.query('gridscroller')[1];
                        		scrollY.scrollByDeltaX(width-(clientWidth-mouseClickX));
                        		scrollY.lastScrl = scrollY.scrollEl.dom.scrollLeft;
                        	}
                            e.stopPropagation();
                        },
                        click: function(e)
                ...
                for(var ci = 0; ci < fca.length; ci++)
                {
                    ...
                    if(fc.xtype == 'monthdatefield')
	                    Ext.apply(fc, {
	                        listeners:{
	                    		'afterChangeValue' : function(field){
                    ...
                    Ext.apply(fc, {
                        ...
                        fieldStyle: 'background: #f4f4f4;color:blue;',
    ...
    changeFont:function(arr,columns){
    	 for(var i in columns){
     		if(arr.indexOf(columns[i].dataIndex)>-1){
     			...
     			columns[i].textEl.dom.style.fontWeight='600';
   ...
   applyFilters: function(isFirst)
		        	...
		        	if(mm.menu&&mm.menu.isHidden()){
		            	...
		            	if(mm.filterType == ''){
		            		...
		            		if(value != "" && column.textEl.dom.style.fontStyle === "italic"){
		            			item.labelEl.dom.style.background = '#A5A0A0';
		            		}
		            		...
		            		if(value!=''){
		            			field.labelEl.dom.style.background = '#A5A0A0';
		            		}
	            		}
		            	mm.onBlur();
		            }
		        	...
		        	if(this.menu&&this.menu.isHidden()){
		            	...
		            	for(var i in this.menu.items.items){
		            		...
		            		if(i==6){
		            			...
		            			item.items.items[0].labelEl.dom.style.background = '';
		            		}else{
		            			item.setValue('');
		            			item.labelEl.dom.style.background = '';
		            		}
		            	}
		            	...
		            	if(value.indexOf('>=')==0){
		            		...
		            		it.items[6].items.items[0].labelEl.dom.style.background = '#A5A0A0';
		            	}else if(value.indexOf('=')>-1){
    ...
    menuKeyEvent:function(field,filterType,owner){
    	...
    	if(filterType=='null'){
	    	...
	    	owner.filterSelect = true;
	    	if(value==''){
	    		field.labelEl.dom.style.background = '';
	    		owner.emptyText = '';
