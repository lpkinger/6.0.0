Ext.define('erp.view.pm.atp.ATPOpDetailGridPanel6',{ 
	extend: 'Ext.grid.Panel', 
	requires: ['erp.view.core.plugin.CopyPasteMenu'],
	alias: 'widget.ATPOpDetailGridPanel6',
        		...
        		var data = res.data != null ? Ext.decode(res.data.replace(/,}/g, '}').replace(/,]/g, ']')) : [];//一定要去掉多余逗号，ie对此很敏感        		
            	   ...
            	   Ext.Ajax.request({//拿到grid的columns
                   	...
                   	params: {
                   		caller: caller,
                   		condition:condition,
                   		fields:'ad_grossqty#ad_usedonhand#ad_usedonorder#ad_netqty',
                   		_noc:1
                   	},
                   	method : 'post',
                   	callback : function(options,success,response){
                   		...
                   		if(res.exception || res.exceptionInfo){
                   			...
                   			if(text){
                   				text.setText('毛需求 :'+obj.ad_grossqty +'净需求:'+obj.ad_netqty+' 库存分配:'+obj.ad_usedonhand+' 在途分配:'+obj.ad_usedonorder);
                   			}else {
                   			Ext.getCmp('pagingtoolbar6').insert(16,{
                  				xtype:'tbtext',
                  				id:'text6',
                  				text:'毛需求 :'+obj.ad_grossqty +'净需求:'+obj.ad_netqty+' 库存分配:'+obj.ad_usedonhand+' 在途分配:'+obj.ad_usedonorder
                  			});
                   			}
                   		}
                   	}
