Ext.define('erp.view.fa.arp.vmQuery.GridPanel',{ 
	extend: 'Ext.grid.Panel', 
	alias: 'widget.erpVmQueryGridPanel',
	id: 'vmquerygrid', 
 	emptyText : '无数据',
    columnLines : true,
    autoScroll : true,
	...
	tbar: {padding:'0 0 5 0',defaults:{margin:'0 5 0 0'},items:[{
    	...
    	handler: function(btn){
			var grid = Ext.getCmp('vmquerygrid');
			grid.BaseUtil.exportGrid(grid, '应付总账');
    	}
	},{
		name: 'refresh',
	...
	listeners: {
        ...
        'afterrender':function(){
        	var me = this;
        	var lockGrid = me.lockedGrid;
        	var normalGrid = me.normalGrid;
        	
        	lockGrid.addListener('itemclick',function(grid,record){
            	if(Number(record.data.vm_id)>0){
            		me.FormUtil.onAdd('showVmDetail'+record.data.vm_id, "查询明细("+record.data.vm_id+")", 'jsps/fa/arp/showVmDetail.jsp?showtype='+record.data.vm_showtype+'&vmid='+record.data.vm_id+'&currency='+record.data.vm_currency+'&vendcode='+record.data.vm_vendcode+'&vendname='+record.data.ve_name+'&yearmonth='+record.data.vm_yearmonth+'&chkumio='+me.chkumio);
            	}
            });
        	normalGrid.addListener('itemclick',function(grid,record){
            	if(Number(record.data.vm_id)>0){
                	me.FormUtil.onAdd('showVmDetail'+record.data.vm_id, "查询明细("+record.data.vm_id+")", 'jsps/fa/arp/showVmDetail.jsp?showtype='+record.data.vm_showtype+'&vmid='+record.data.vm_id+'&currency='+record.data.vm_currency+'&vendcode='+record.data.vm_vendcode+'&vendname='+record.data.ve_name+'&yearmonth='+record.data.vm_yearmonth+'&chkumio='+me.chkumio);
        		}
	...
	viewConfig: { 
        getRowClass: function(record) {
        	if(record.get('vm_showtype')=='1'){
        		return 'custom';
        	}else if(record.get('vm_showtype')=='3'){
        		return 'custom-blank';
        	}else{
        		return 'custom-alt';
        	}
