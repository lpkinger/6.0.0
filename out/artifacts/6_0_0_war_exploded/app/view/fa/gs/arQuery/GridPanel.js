Ext.define('erp.view.fa.gs.arQuery.GridPanel',{ 
	extend: 'Ext.grid.Panel', 
	alias: 'widget.erpArQueryGridPanel',
	id: 'arquerygrid', 
 	emptyText : '无数据',
    columnLines : true,
    autoScroll : true,
	...
	tbar: {margin:'0 0 5 0',style:{background:'#fff'},defaults:{margin:'0 10 0 0'},items:[{
    	...
    	handler: function(btn){
			...
			grid.BaseUtil.exportGrid(grid, '银行存款总账');
    		}
	},{
		name: 'refresh',
        	...
        	for(var i in grids) {
        		if(grids[i]) {
        			grids[i].addListener('itemclick',function(grid,record){
                    	if(Number(record.data.am_id)>0){
                    		me.FormUtil.onAdd('showArDetail'+record.data.am_id, "查询明细("+record.data.am_id+")", 'jsps/fa/gs/showArDetail.jsp?showtype='+record.data.am_showtype+'&amid='+record.data.am_id+'&currency='+record.data.am_currency+'&accountcode='+record.data.ca_code+'&accountname='+record.data.ca_description+'&yearmonth='+record.data.am_yearmonth);
                    	}
                    });
        		}
        	}
	...
	viewConfig: { 
        getRowClass: function(record) {
        	if(record.get('am_showtype')=='1'){
        		return 'custom';
        	}else if(record.get('am_showtype')=='3'){
        		return 'custom-blank';
        	}else{
        		return 'custom-alt';
        	}
