Ext.define('erp.view.common.query.Form',{ 
	...
	onQuery: function() {
		...
		//lidy 2017120467 Query界面添加afterQuery方法，支持业务逻辑配置
		form.afterQuery(caller,condition);
	},
	...
	afterQuery: function(call, cond) {
		Ext.Ajax.request({
			url: basePath + 'common/form/afterQuery.action',
			params: {
				caller: call,
				condition: cond
			},
			...
			callback: function(opt, s, r) {
				...
				if(rs.exceptionInfo) {
					try{
						var returnStr = Ext.JSON.decode(rs.exceptionInfo);
						Ext.Object.each(returnStr,function(key,value){
							if(key=='exceptionInfo'){
								showError(value);
							}else{
								var dom = Ext.getCmp(key);
								if(dom){
									dom.setValue(value);
								}
							}
