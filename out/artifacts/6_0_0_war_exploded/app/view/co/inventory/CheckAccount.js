Ext.define('erp.view.co.inventory.CheckAccount',{ 
	extend: 'Ext.Viewport', 
	layout: 'anchor', 
	hideBorders: true, 
			...
			items: [{
				...
				store: Ext.create('Ext.data.Store',{
					...
					data: [{
						...
						detno: 5,
						type: 'co_chk_e',
						value: '委外领料单、委外补料单、委外退料单、委外验收单、委外验退单存货金额是否与总账科目一致'
					},{
						action: 'co/inventory/chk_f.action',
						detno: 6,
						type: 'co_chk_f',
						value: '其它出/入库单存货金额是否与总账科目一致'
					},{
						action: 'co/inventory/chk_g.action',
						detno: 7,
						type: 'co_chk_g',
						value: '盘盈、盘亏、报废单与相应凭证存货科目是否与总账科目一致'
					},{
					...
					},*/
					{
						...
						detno: 9,
						type: 'co_chk_j',
						value: '发货单、退货单存货金额是否与总账科目一致'
					},{
						action: 'co/inventory/chk_k.action',
						detno: 10,
						type: 'co_chk_k',
						value: '期末汇总表【期末结存金额】与总账对应存货科目余额是否一致'
					},{
						action: 'co/inventory/chk_l.action',
						detno: 11,
						type: 'co_chk_l',
						value: '存货月结表：期初数量是否与上月期末数量一致'
					},{
						...
						detno: 18,
						type: 'co_chk_s',
						value: '是否存在总账直接制作存货科目凭证或非出入库单、应付发票、主营业务成本结转制作的存货科目凭证'
					},{
