...
Ext.define('erp.view.b2c.common.b2cQueryGrid',{ 
	...
	listeners: {
		...
		'headerfilterchange': function(grid, filters) {
			...
			if(filters){
				...
				if(bool){
					grid.store.pageSize = grid.store.totalCount;			
				}
			}	
        	return true;
        },
	...
	getGridColumnsAndStore: function(grid, url, param, no){
		...
		Ext.Ajax.request({//拿到grid的columns
        	...
        	callback : function(options,success,response){
        		...
        		var gridcolumn = grid.gridcolumns;
        		if(me.columns && me.columns.length > 2){
        			...
        			if(gridcolumn){
        				Ext.each(gridcolumn, function(column, y){
            			...
            			//store
            			me.store = me.setDefaultStore(null, grid.datafields);
            			//view
                		if(me.selModel && me.selModel.views == null){
                			me.selModel.views = [];
                		}
                		if(grid.dbfinds.length > 0){
                			me.dbfinds = grid.dbfinds;
                		}
        				//toolbar
                		me.setToolbar(gridcolumn);
                		//reconfigure store&columns
                		grid.reconfigure(me.store, gridcolumn);
            		}
        		}
        	/*}
        });*/
