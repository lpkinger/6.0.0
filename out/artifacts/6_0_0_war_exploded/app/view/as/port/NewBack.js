Ext.define('erp.view.as.port.NewBack', {
	extend : 'Ext.Viewport',
	layout : 'anchor',
	hideBorders : true,
		...
		Ext.apply(me, {
			items : [{
							...
							listeners:{
								selectionchange:function(selModel, selected, options){
									...
									if(record && selModel.isSelected(record)) {
										var code = selModel.lastSelected.get('NEWFITTINGBACK_NO');
										if(!Ext.isEmpty(code)) {
						...
						store : new Ext.data.Store({
							fields : [ 'NEWFITTINGBACK_NO', 'STORE_NAME', 'BACK_DAY',
									'SHENQING_OP', 'OBJECTIVE_STORE',
									'BILL_DETAIL', 'BEIZHU' ],
							proxy : {
								type : 'ajax',
								url : basePath + 'as/port/getNewBack.action',
								reader : {
									type : 'json',
									root : 'data'
								}
						...
						store : new Ext.data.Store({
							fields : [ 'ID', 'FITTING_CODE', 'FITTING_NAME',
							           'UNIT', 'KEHEXIAO', 'GOODORBAD', 
							           'BEIZHU'],
							proxy : {
								type : 'ajax',
								url : basePath + 'as/port/getNewBackDetail.action',
								reader : {
									type : 'json',
									root : 'data'
								}
