define([ 'app/app','common/services','service/Purc','service/SupportServices' ], function(app) {
	app.register.controller('InMMwaitSubmitCtrl',['$scope','$rootScope','$stateParams','$filter', 'ngTableParams','Online','PurcOrderItem','toaster','Ring', function($scope, $rootScope,$stateParams,$filter, ngTableParams,Online,PurcOrderItem,toaster,Ring){
		...
		if($scope.grid){
		 ...
		 $scope.deleteAll = function (){//全部删除
		 	if(confirm("确定删除全部？")){
		 		 angular.forEach($rootScope.orders, function(value, key) {//反写剩余数
