...
var requiredFields = null,instance = null;
	...
	if (bool) {//获取编辑的字段值
		...
		$.ajax({
			...
			success: function(result){	
			   ...
			   window.location.href = url.substr(0, url.indexOf('?') + 1) + 'nodeId=' + getUrlParam('nodeId');
			   /*$.each(d,function(key,field){//修改数据
				 if(d[key] != bill[key])					
					   bill[key] = d[key];	
				});*/
			},
...
$(document).ready(function() {
	...
	var parseBillDetail = function(content,i,detail,field,keyValue,keyName){//field 可编辑的字段，keyValue关联主表字段值		
		...
		var condition ;
		if(caller == "PayPleaseDet"){
			condition = mad_code +"=(select ppd_id from (select ppd_id,row_number() over (order by ppd_detno) rn from paypleasedetail where ppd_ppid="+keyValue+") where rn < 2)"
		}else if(caller == ""){
			condition  = mad_code +"=(select ppd_id from (select ppd_id,row_number() over (order by ppd_detno) rn from paypleasedetail where ppd_ppid="+keyValue+") where rn < 2)";
		}else{
		...
		$.ajax({
			...
			success: function(result){		
				...
				for(var n=0;n<result.columns.length;n++){
					...
					if($.inArray(result.fields[n].name.toLowerCase(), field) > -1){
						...
						if(result.columns[n].xtype == 'numbercolumn'){ //数字类型
						    edittype = 'text';
						    number = true;
						}else if(result.columns[n].xtype == '' && result.columns[n].editor.xtype == 'dbfindtrigger'){//dbfind
							 edittype = 'custom';
							 var resultDatas = {};
							...
							edittype = 'text';
						}else if(result.columns[n].xtype == 'combocolumn'){//下拉框类型
							edittype='select';
						}
					}
					if(result.fields[n].name == detno_code){
					    ...
					    $.each(data,function(d,va){
					    	values = va.display+":"+va.value;
					    });
						colModel.push({"label":result.columns[n].header,"name":result.fields[n].name,"index":result.fields[n].name,"sorttype":result.fields[n].type,"width":result.columns[n].width,"editable":editable,"edittype":edittype,"editoptions":{value:values},hidden:hidden});						
				...
				jQuery('#grid'+i).jqGrid({
					...
					"pager":"#pager"+i,
					"editurl":basePath+"common/processUpdate.action"
				});
				jQuery('#grid'+i).jqGrid('navGrid','#pager'+i,{add:false,edit:false,del:false},{reloadAfterSubmit:false},{},{},{multipleSearch:true}); 					
			},
			error: function(xhr){
				setLoading(false);
				if(xhr.responseText) {
	...
	var parseBillMain = function(main,button,cond,caller) {
      ...
      var html = '<form  method="post" id="mainForm"> ' +
      		      ' <table class="table table-condensed table-bordered table-striped"><colgroup><col width="45%"></col><col width="55%"></col></colgroup>';
      if(bo){//有可编辑的增加保存按钮
      	var cd = cond.split("=");
      	html +='<div class="list-group"> <div class="text-center col-xs-8" id="tips" style="color:#FF0000;"> </div>';
      	html +="<div class='text-right'><button type='button' class='btn-sm' id='"+button.jb_buttonid +"' name='"+button.jb_buttonname+ "' onclick=\"saveForm('"+cd[0]+"',"+cd[1]+",'"+caller+"');\">"+button.jb_buttonname+"</button></div></div>";
      }
		      		...
		      		if(d.groups[i].id != null && d.groups[i].xtype != "hidden" && d.groups[i].fieldLabel != null){
						...
						if(!d.groups[i].readOnly){//可编辑字段
							...
							if(d.groups[i].xtype == 'combo'){//下拉框
								html +='<td class="text-center"><select name="'+d.groups[i].id+'" class="cellElement form-control" style="'+d.groups[i].fieldStyle+'"'+ (d.groups[i].logic ? 'logic="' + d.groups[i].logic + '"' : '') +(d.groups[i].allowBlank ? '' : 'required') +'>';
								html +='<option value=""></option>';
								$.each(d.groups[i].store.data,function(key,da){
									if(bill[d.groups[i].id] == da.value){
							    		html +='<option value="'+da.value+'" selected="selected">'+da.display+'</option>';
									}else{
							    ...
							    html += '<td class="text-center">' +
									'<input type="text" id="'+d.groups[i].id+'" style='+d.groups[i].fieldStyle+
											' name='+ d.groups[i].id+' style='+d.groups[i].fieldStyle +
											' class="cellElement form-control" dbfind value="'+strData +'"'+ (d.groups[i].logic ? 'logic="' + d.groups[i].logic + '"' : '') +(d.groups[i].allowBlank ? '' : 'required') +'/>' +
									'<input type="text" id="'+ d.groups[i].secondname+'" readonly name='+ d.groups[i].secondname+
										   ' class="setInput " value="'+seData+'"/>'+
									 '</td>';
								...
								html += '<td class="text-center">' +
									'<input type="text" id='+ d.groups[i].id+' style='+d.groups[i].fieldStyle+
											' name='+ d.groups[i].id+' style='+d.groups[i].fieldStyle +
											' class="cellElement form-control" value="'+strData +'"'+ (d.groups[i].logic ? 'logic="' + d.groups[i].logic + '"' : '') +(d.groups[i].allowBlank ? '' : 'required') +'/>' +
									'</td>';
							}
						}else{//不可编辑字段
							strData = changeEscapeCharacter(bill[d.groups[i].id]);
							if(d.groups[i].xtype == "multifield"){//MT类型
							   ...
							   html += '<td class="text-center">' +
									' <input type="text" id='+ d.groups[i].id+' readonly name='+ d.groups[i].id+
										' class="setInput" value="'+strData +'"'+(d.groups[i].logic ? 'logic="' + d.groups[i].logic + '"' : '')+'/>' +
										' <input type="text" id='+ d.groups[i].secondname+' readonly name='+ d.groups[i].secondname+
										' class="setInput" value="'+seData +'"/>' +
								   '</td>';
							}else{
	      		...
	      		if(d.id != null && d.xtype != "hidden" && d.fieldLabel != null){
						...
						if(d.xtype == "multifield"){ //MT
						 seData = changeEscapeCharacter(bill[d.secondname]);
						  html += '<td class="text-center">' +
								'<input type="text" id='+ d.id+' readonly class="setInput" name='+ d.id+
										' value="'+strData +'"'+(d.logic ? 'logic="' + d.logic + '"' : '')+'/>' +
								'<input type="text" id='+ d.secondname+' readonly class="setInput" name='+ d.secondname+
										' value="'+seData +'"/>' +
								'</td>';		
						}else{							
							html += '<td class="text-center">' +
	...
	var parseMainPoint = function(points) {
		...
		$.each(points, function(i, p){
			...
			if(p.type === 'B') {//boolean
				...
				html += '<div class="pull-right"><select class="form-control input-xs" '+(p.required ? 'required' : '')+' ><option></option>';
