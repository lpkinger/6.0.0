...
App.property.MaxWindow = Ext.extend(Ext.Window, {
    ...
    layout: 'fit',
    stateful: false,
    draggable :false,  //不允许拖动 bug zhouy
    closable: false, 
    width: 500,
    height: 400,
...
App.property.PropertyManager = Ext.extend(Object, {
    ...
    initSelectionListener2: function(editor,data) {
    	var k=editor
        this.selectionListener = new Gef.jbs.ExtSelectionListener(this);
        editor.addSelectionListener(this.selectionListener);
        this.selectionListener.setEditor(editor);
        var model = this.selectionListener.getModel();
         model.dom.setAttribute("data",data);
        this.updateForm(model);
    },
    selectSpecialNode:function(editor,editPart){
    	var editmodel=editPart.getModel();
    	this.selectionListener = new Gef.jbs.ExtSelectionListener(this);
        editor.addSelectionListener(this.selectionListener);
        this.selectionListener.setEditor(editor);
        this.selectionListener.selectNode(editPart);
        var model = this.selectionListener.getModel();
        this.updateForm(model);
    },
    initSelectionListener3:function(editor,constructor){
    	 this.selectionListener = new Gef.jbs.ExtSelectionListener(this);
         editor.addSelectionListener(this.selectionListener);
         this.selectionListener.setEditor(editor);
         var model = this.selectionListener.getModel();
         ...
         model.dom.setAttribute("constructor",constructor);
         this.updateForm(model);
    }
});
