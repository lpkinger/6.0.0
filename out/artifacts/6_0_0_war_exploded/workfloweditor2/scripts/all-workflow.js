...
App = {
    ...
    initEditor: function() {
     ...
     Ext.Ajax.request({
	   	...
	   	url: basePath+'oa/flow/getDefineInstance.action',
	   	success: function(response) {
	   		var o = Ext.decode(response.responseText);
	   		if(o.success){
	   			var Object = {
		   			flowName : o.data[0].FD_SHORTNAME,
		   			flowCaller : o.data[0].FD_CALLER,
		   			flowRemark : o.data[0].FD_REMARK
	   			}
     ...
     var xml = "<process  xmlns='http://jbpm.org/4.4/jpdl' name='sale'> "+
    		"<start g='253,67,48,48' name='START'>"+
    		"<transition to='END' name = 'to END'/></start>"+
    		"<end g='247,239,48,48' name='END'/></process>";
    ...
    createEast: function() {
        ...
        //zhouy 2013-08-27  点击节点属性编辑界面 会点中center 流程设置
...
App.CanvasPanel = Ext.extend(Ext.Panel, {   //中间画板 2013-3-6 10:31:05 wuw
	id:'canvaspanel',
	xmlValidate:function(){        //jpdl规范校验  2013-3-6 10:31:12 wuw
		  ...
		  try //Firefox, Mozilla, 浏览器
		   ...
		   /* parser=new DOMParser();
		    ...
		    for(var i=0;i<tasks.length;i++){        // 对任务节点的校验……
		    	...
		    	if(transitions.length!=2){         //任务节点只能有两条transition元素
		    	...
		    	for(var j=0;j<transitions.length;j++){
		    		...
		    		console.log(name);
		    		if(name!='同意'&&name!='不同意'){   //任务节点两条transition元素只能为'同意'和'不同意'
		    	...
		    	var att= tasks[i].getAttribute("sqlAssignee");
		    	if(att){
		    		var sqlAssignee = new String(att);
			    	if(sqlAssignee.indexOf("#")<0 && sqlAssignee.indexOf("@")<0 && sqlAssignee.indexOf("$")<0){
			    		Ext.Msg.alert("温馨提示","任务".fontcolor("Red").fontsize(5)+new String(tasks[i].getAttribute('name')).fontcolor("Red").fontsize(5)+" 的SQL取人表达式设置错误 !");
		    			return false;
			    	}
		    	}
    ...
    initComponent: function() {
        ...
        this.tbar = new Ext.Toolbar([{
            ...
            iconCls: 'tb-new',
            handler: function() {
            	 //新建前选中 面板 用于得到面板定义的panel 2012-08-15 zhouy
            	//var constructor={'pr_constructor' : en_uu};
                        ...
                        buttons: [{
                            ...
                            handler: function() {
                             ...
                             /*   var editor = new Gef.jbs.ExtEditor();
               		         ...
               		         Gef.activeEditor = editor;
               		         me.propertyManager.initSelectionListener(editor);*/
							 //2013-3-5 15:23:10 wuw 以下代码是否多余，还有问题？不知道想干啥
                             Ext.getCmp("FirstForm").items.items[0].setValue(data.flowName);
               		         Ext.getCmp("FirstForm").items.items[1].setValue(data.flowCaller);
               		         Ext.getCmp("FirstForm").items.items[4].setValue(data.flowRemark);
                            },
                            scope: this
                        }, {
                            text: '取消',
...
//                commandStack.redo();
    ...
    showWindow: function() {   //详细配置窗口 2013-3-6 10:31:38 wuw
    ...
    removeSelected: function() {
        ...
        if (edge != null) {
            ...
            for(var i=0;i<nodes.length;i++){
            	if(nodes[i].incomingConnections!=null&&nodes[i].incomingConnections.length>0){
            		for(var j=nodes[i].incomingConnections.length-1;j>=0;j--){
            			 this.executeCommand(nodes[i].incomingConnections[j], request);       			
            		}
            	}
            	if(nodes[i].outgoingConnections!=null&&nodes[i].outgoingConnections.length>0){
