/*
 * 仅用于查看流程图 只展示视图界面 不可编辑 流程图会跟随节点渲染颜色
 */
Ext.ns('App');
						...
						if(res.data.length!=0){
							...
							for(var x=0;x<data.length;x++){
				                ...
				                for(var i=0;i<childrens.length;i++){
				   		   		     ...
				   		   		     //普通渲染
				   		   		     if(childrens[i].text==data[x].FI_NODENAME){
				   		   		     	 childrens[i].renderOver = true;
				   		   		     	 if(figure.rectEl){
             	                		 	figure.rectEl.strokwidth= 1;
             	                		 	figure.rectEl.style.fill='#7FFFAA';
             	                		 	figure.rectEl.style.stroke='#64a1ed';
				   		   		     	 }
					            		 figure.el.style.fill='#0E7D9F';
					            		 me.propertyManager.selectSpecialNode(Gef.activeEditor,childrens[i].editPart);
					            		 //给节点间的线段渲染颜色
					            		 for(var j=0;j<childrens[i].outgoingConnections.length;j++){
					            		 	var figure=childrens[i].outgoingConnections[j].editPart.figure;
					            		 	if(data[x].FO_NAME=='commit'){
					            		 		data[x].FO_NAME='提交'
					            		 	}
					            		 	if(figure.name == data[x].FO_NAME){
					            		 		/*figure.el.style.fill='#15B10E';*/
					            		 		figure.el.style.stroke='#29EDAE';
					            		 		childrens[i].outgoingConnections[j].renderOver = true;
					            		 	}
					            		 }
					            		 //给进行中的节点渲染颜色
					            		 if(x==(data.length-1)){
					            		 	 for(var i=0;i<childrens.length;i++){
					            		 	 	if(!childrens[i].renderOver){
					            		 	 		for(var j=0;j<childrens[i].incomingConnections.length;j++){
					            		 	 			if(childrens[i].incomingConnections[j].renderOver){
					            		 	 				var figure=childrens[i].editPart.figure;
					            		 	 				if(figure.rectEl){
					             	                		 	figure.rectEl.strokwidth= 1;
					             	                		 	figure.rectEl.style.fill='#F08080';
					             	                		 	figure.rectEl.style.stroke='#64a1ed';
									   		   		     	}
									   		   		     	figure.el.style.fill='#0E7D9F';
					            		 					me.propertyManager.selectSpecialNode(Gef.activeEditor,childrens[i].editPart);
	...
	listeners: {
        render: function(p){
          p.body.on('scroll', function(){
            var viewgrid = parent.Ext.getCmp('view');
            if(viewgrid&&!viewgrid.collapsed){
            	viewgrid.collapse();
            }
          }, p);
        }
