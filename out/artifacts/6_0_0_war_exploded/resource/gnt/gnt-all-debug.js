...
Ext.define("Gnt.plugin.TaskContextMenu", {
	...
	copyTask : function(c) {
	    ...
	    //2012-11-29 zhouy
	    a.set("prjplanid",projectplandata.prjplan_id);
	    a.set("prjplanname",projectplandata.prjplan_prjname);
	    a.set('taskcode',BaseUtil.getSerialNumber('M'+projectplandata.prjplan_id,'ProjectTask'));  			
		a.setPercentDone(0);
	...
	deleteTask : function() {
	    ...
	    if(a.items[0].data.type==0){
		this.grid.taskStore.remove(a.items);
		}else showError('该任务是里程碑不能删除');
	},
	scanDetails:function(){
	   var a = this.grid.getSelectionModel().selected;
	   var data=a.items[0].data;
	   if(!data.leaf){
	   showError('请选择子任务查看');
	   return ;
	   }
	    ...
	    var win = new Ext.window.Window(
				{
					...
					items : [ {
						...
						frame : true,
						layout : 'fit',
						html : '<iframe id="iframe_'+ caller+ '" src="'+ basePath+ 'jsps/plm/record/specialrecord.jsp?gridCondition='+condition+'" height="100%" width="100%" frameborder="0" scrolling="no"></iframe>'
					} ],

				});
		win.showAt(window.innerWidth/2-450,10);
	},
				...
				return ["->", {
					...
					if(this.taskId){
						Ext.Function.defer(this.onGridClose, Ext.isIE
						       ...
						       //Ext.Msg.alert('','新建任务不允许分配资源!',me.collapse);
						        me.collapse
						       showError('新建任务未保存不允许分配资源!');
