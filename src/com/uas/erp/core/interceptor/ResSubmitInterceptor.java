...
/**
 * 自定义拦截器 查看用户是否有反提交的权限
 * 
 * @author yingp
 */
public class ResSubmitInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			...
			if (InterceptorUtil.noControl(req))
				return true;
			boolean bool = checkJobPower(caller, PositionPower.RESSUBMIT, employee);
			if (!bool) {
				bool = powerDao.getSelfPowerByType(caller, PersonalPower.RESSUBMIT, employee);// 个人权限
				if (!bool) {
					BaseUtil.showError("ERR_POWER_021:您没有<反提交>该单据的权限!");
				} else {
					Integer id = Integer.parseInt(req.getParameter("id"));
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESSUBMIT_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_022:您没有<反提交他人>单据的权限!");
						} else {
							return true;
						}
					}
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.RESSUBMIT_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESSUBMIT_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_022:您没有<反提交他人>单据的权限!");
