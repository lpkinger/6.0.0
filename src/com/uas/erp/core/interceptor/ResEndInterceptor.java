...
/**
 * 自定义拦截器 查看用户是否有反操作的权限
 * 
 * @author yingp
 */
public class ResEndInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			boolean bool = checkJobPower(caller, PositionPower.RESEND, employee);
			if (!bool) {
				bool = powerDao.getSelfPowerByType(caller, PersonalPower.RESEND, employee);// 个人权限
				if (!bool) {
					BaseUtil.showError("ERR_POWER_017:您没有<反操作>该单据的权限!");
				} else {
					Integer id = Integer.parseInt(req.getParameter("id"));
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESEND_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_018:您没有<反操作他人>单据的权限!");
						} else {
							return true;
						}
					}
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.RESEND_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESEND_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_018:您没有<反操作他人>单据的权限!");
