...
/**
 * 自定义拦截器 查看用户是否有反禁用的权限
 * 
 * @author yingp
 */
public class ResBannedInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			boolean bool = checkJobPower(caller, PositionPower.RESBANNED, employee);
			if (!bool) {
				bool = powerDao.getSelfPowerByType(caller, PersonalPower.RESBANNED, employee);// 个人权限
				if (!bool) {
					BaseUtil.showError("ERR_POWER_015:您没有<反禁用>该单据的权限!");
				} else {
					Integer id = Integer.parseInt(req.getParameter("id"));
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESBANNED_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_016:您没有<反禁用他人>单据的权限!");
						} else {
							return true;
						}
					}
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.RESBANNED_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESBANNED_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_016:您没有<反禁用他人>单据的权限!");
