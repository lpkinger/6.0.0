...
/**
 * 自定义拦截器 查看用户是否有反过账的权限
 * 
 * @author yingp
 */
public class ResPostInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			boolean bool = checkJobPower(caller, PositionPower.RESPOST, employee);
			if (!bool) {
				bool = powerDao.getSelfPowerByType(caller, PersonalPower.RESPOST, employee);// 个人权限
				if (!bool) {
					BaseUtil.showError("ERR_POWER_019:您没有<反过账>该单据的权限!");
				} else {
					Integer id = Integer.parseInt(req.getParameter("id"));
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESPOST_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_020:您没有<反过账他人>单据的权限!");
						} else {
							return true;
						}
					}
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.RESPOST_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.RESPOST_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_020:您没有<反过账他人>单据的权限!");
