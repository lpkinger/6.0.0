		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			...
			if (!bool) {
				...
				if (!bool) {
					...
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.AUDIT_OTHER, employee);
						if (!bool) {
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.AUDIT_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.AUDIT_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_003:您没有<审核他人>单据的权限!");
