...
public class PostInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse resp, Object handler) throws Exception {
		...
		if (caller != null && employee != null && !"admin".equals(employee.getEm_type()) && !InterceptorUtil.isOpenSys(req)) {
			boolean bool = checkJobPower(caller, PositionPower.POST, employee);
			if (!bool) {
				bool = powerDao.getSelfPowerByType(caller, PersonalPower.POST, employee);// 个人权限
				if (!bool) {
					BaseUtil.showError("ERR_POWER_009:您没有<过账>该单据的权限!");
				} else {
					Integer id = Integer.parseInt(req.getParameter("id"));
					if (id != null) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.POST_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_010:您没有<过账他人>单据的权限!");
						} else {
							return true;
						}
					}
				...
				Integer id = Integer.parseInt(req.getParameter("id"));
				if (id != null) {
					bool = powerDao.getOtherPowerByType(caller, id, PositionPower.POST_OTHER, employee);
					if (!bool) {
						bool = powerDao.getOtherSelfPowerByType(caller, id, PersonalPower.POST_OTHER, employee);
						if (!bool) {
							BaseUtil.showError("ERR_POWER_010:您没有<过账他人>单据的权限!");
