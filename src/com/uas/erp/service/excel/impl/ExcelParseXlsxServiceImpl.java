...
@Service
public class ExcelParseXlsxServiceImpl implements ExcelParseXlsxService{
	...
	private void dealColWidth(XSSFSheet sheet,ExcelSheet excelSheet, int col) {
	...
	@Transactional(propagation=Propagation.REQUIRED)
	private void dealRowHeight(XSSFSheet sheet,ExcelSheet excelSheet, XSSFRow row) {
	...
	public void dealMergedRegion(XSSFSheet sheet, XSSFCell cell, ExcelSheet excelSheet) {
	...
	//保存Cell数据
	@Transactional(propagation=Propagation.REQUIRED)
	private void saveCell(XSSFCell cell,ExcelSheet excelSheet) {
		ExcelCell excelCell = new ExcelCell();
		excelCell.setCellid(baseDao.getSeqId("EXCELCELL_SEQ"));
		excelCell.setCellsheetid(excelSheet.getsheetid());
...
//					BaseUtil.showError("存在错误的公式！");
	...
	private String createCellcontent(XSSFCell cell){
