...
public class ExcelParseServiceImpl implements ExcelParseService {
	...
	@Override
	public int parseExcel(MultipartFile file, int subof,Employee employee) {
		String filename = file.getOriginalFilename();
		String suffix = filename.substring(filename.lastIndexOf('.') + 1);
		if (suffix.toUpperCase().equals("XLS")) {
			return parseXls(file, subof,employee);
		}else {
			return parseXlsx(file, subof,employee);
		}
	}
	
	private int parseXlsx(MultipartFile file, int subof, Employee employee) {
		
		return 0;
	}

	@Transactional(propagation=Propagation.REQUIRED)
	public int parseXls(MultipartFile file, int subof,Employee employee) {
	                ...
	                if (row != null) {
	                    ...
	                    for (int col = 0; col < row.getLastCellNum(); col++) {
	                    	...
	                    	if (row.getCell(col) != null) {
	                    		...
	                    		if (bool) {
									dealMergedRegion(sheet,cell,excelSheet,workbook);
								}else {
									saveCell(cell, excelSheet,workbook);
								}
	                        }
	                    }
	                }
	...
	private void dealColWidth(Sheet sheet,ExcelSheet excelSheet, int col) {
    	Double width = (double) sheet.getColumnWidth(col);
    	Double Dfwidth = (double) (sheet.getDefaultColumnWidth()*256);
    	if (!width.equals(Dfwidth)) {
    		//判断是否已经解析过该Column的宽度;
    		int count = baseDao.getCount("select count(*) from "+excelSheet.getsheetcelltable()+
    		" where cellsheetid="+excelSheet.getsheetid()+" AND  cellrow=0 AND cellcol="+(col+1));
    		if (count==0) {
	...
	@Transactional(propagation=Propagation.REQUIRED)
	private void dealRowHeight(Sheet sheet,ExcelSheet excelSheet, Row row) {
	...
	public void dealMergedRegion(Sheet sheet, Cell cell, ExcelSheet excelSheet,HSSFWorkbook workbook) {
	...
	//保存Cell数据
	@Transactional(propagation=Propagation.REQUIRED)
	private void saveCell(Cell cell,ExcelSheet excelSheet, HSSFWorkbook workbook) {
		ExcelCell excelCell = new ExcelCell();
		excelCell.setCellid(baseDao.getSeqId("EXCELCELL_SEQ"));
		excelCell.setCellsheetid(excelSheet.getsheetid());
		excelCell.setCellcol(cell.getColumnIndex()+1);
		excelCell.setCellrow(cell.getRowIndex()+1);
		excelCell.setCellcontent(createCellcontent(cell,workbook));
	...
	private String createCellcontent(Cell cell, HSSFWorkbook workbook){
