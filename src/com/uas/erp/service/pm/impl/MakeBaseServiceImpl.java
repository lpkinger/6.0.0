...
public class MakeBaseServiceImpl implements MakeBaseService {
	...
	public void deleteMakeBase(int ma_id, String caller) {
		...
		int maQty = baseDao.queryForObject("select ma_qty from make where ma_id = ? ", Integer.class, ma_id);
		// 执行删除前的其它逻辑
		handlerService.beforeDel(caller, ma_id);
		// 删除Make
		/* makeDao.deleteMake(ma_id); */
		//来源请购单则更新请购单明细已转数
		Object ma_adid = baseDao.queryForObject("select ma_adid from make where ma_id = ? ", Integer.class, ma_id);
		...
		//返还已转数
		if(ma_adid!=null&&maQty!=0){
			baseDao.execute("update applicationdetail set ad_yqty = ad_yqty-"+maQty+" where ad_id = "+ma_adid);
			applicationservice.updateApplicationTurnMakeStatus(ma_adid);
		}
	}

	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void updateMakeBase(String formStore, String gridStore, String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		List<Map<Object, Object>> gstore = BaseUtil.parseGridStoreToMaps(gridStore);
		...
		//工单请购来源
		Object ma_adid = baseDao.getFieldDataByCondition("make", "ma_adid", " ma_id="+store.get("ma_id"));
		if(ma_adid !=null){//有请购来源的不能修改物料编号
			if(!baseDao.checkByCondition("make", "ma_id = "+store.get("ma_id")+" and ma_prodcode <> '"+store.get("ma_prodcode")+"'")){
				BaseUtil.showError("有请购来源的不能修改物料编号!");
			}
		}
		// 只能修改[在录入]的资料!
		Object status = baseDao.getFieldDataByCondition("Make", "ma_statuscode", "ma_id=" + store.get("ma_id"));
		StateAssert.updateOnlyEntering(status);
		int maQtyChange = baseDao.queryForObject("select  "+store.get("ma_qty")+"-ma_qty from make where ma_id = ? ", Integer.class, store.get("ma_id") );
		if(ma_adid!=null&&maQtyChange!=0){
			if(!baseDao.checkByCondition("applicationdetail", "ad_id = "+ma_adid+" and ad_qty-ad_yqty -("+maQtyChange+") < 0")){
				BaseUtil.showError("变更数量不能大于来源请购单明细可转数量");
			}
		}
		// 执行修改前的其它逻辑
		handlerService.beforeSave(caller, store, gstore);
		...
		if(ma_adid!=null&&maQtyChange!=0){
			//制造单变化数
			baseDao.execute("update applicationdetail set ad_yqty = ad_yqty + ("+maQtyChange+") where ad_id = "+ma_adid);
			applicationservice.updateApplicationTurnMakeStatus(ma_adid);
		}
		useDefaultTax(caller, store.get("ma_id"));
		// 修改MakeMaterial
	...
	@Override
	public String[] printMakeBase(int ma_id, String caller, String reportName, String condition) {
		// 已审核才允许打印
	...
	private void calBackQty(String data, String caller) {
		
		List<Map<Object, Object>> map = BaseUtil.parseGridStoreToMaps(data);
		for(Map<Object, Object> m :map){
			//if(Integer.parseInt(m.get("ma_thisqty").toString())!=0){
				String str = baseDao.callProcedure("MM_ASSIGNBACKQTY", m.get("ma_id"),m.get("ma_thisqty"));
				if (str != null && !str.trim().equals("")) {
					// 提示错误信息
					BaseUtil.showError("计算可退料数失败");
				}
			//}
		}
	}
	...
	public void vastCloseMake(String data, String caller) {
		...
		for (Map<Object, Object> m : list) {
			//@add 20180709 如果工单已经结案则无需再判断
			id = m.get("ma_id");
			rs = baseDao.queryForRowSet("select ma_id from make where ma_id=? and ma_statuscode='FINISH'",id);
			if(rs.next()){
				continue;
			}
			...
			if (Sqlstr != null && rs != null && rs.hasNext()) {
				...
				if(baseDao.isDBSetting("usingMakeCraft")){
					baseDao.execute("update MakeCraft set mc_status='" + BaseUtil.getLocalMessage("FINISH") + "',mc_statuscode='FINISH' where mc_maid="+id);
				}
				// 记录操作
				baseDao.logMessage(new MessageLog(SystemSession.getUser().getEm_name(), BaseUtil.getLocalMessage("msg.end"), BaseUtil
	 ...
	 */
	@Override
	public void calOnlineQty(String ids, String caller, String datas) {
		// 计算在制
	...
	public void splitMake(String formdata, String data, String caller) {
		...
		int splitsumqty = 0;
		//当物料委外损耗为空时，不取量产损耗   委外工单BOM用量匹配
		int osLoss = baseDao.getCount("select count(1) from configs where caller='Make' and code='osLossNotUsePrLoss'");
		...
		for (Map<Object, Object> map : maps) {
			...
			if(standardqty){
				baseDao.execute(" merge into makematerial using product on ( mm_prodcode=pr_code)"
								...
								+" ELSE (case when ?>0 then NVL(pr_exportlossrate, 0) else NVL(pr_lossrate, 0) end)  END )* 0.01)+0.4999*power(0.1,nvl(pr_precision,0)),nvl(pr_precision,0)),"
								+" mm_balance=round(mm_oneuseqty*?*(1+ (CASE WHEN ?='OS' AND NVL(pr_exportlossrate,0)>0 THEN NVL(pr_exportlossrate, 0) "
								+" ELSE (case when ?>0 then NVL(pr_exportlossrate, 0) else NVL(pr_lossrate, 0) end) END )* 0.01)+0.4999*power(0.1,nvl(pr_precision,0)),nvl(pr_precision,0))-mm_oneuseqty*? where mm_maid=? and mm_qty>=mm_oneuseqty*?",maqty,maketype,osLoss,maqty,maketype,osLoss,maqty,maid,maqty);			
			}
			// 插入替代料
		...
		if (allmaqty > remainmaqty) {
			...
			if(standardqty){
					baseDao.execute(" merge into makematerial using product on (mm_prodcode=pr_code)"
							...
							+" ELSE (case when ?>0 then NVL(pr_exportlossrate, 0) else NVL(pr_lossrate, 0) end ) END )* 0.01)+0.4999*power(0.1,nvl(pr_precision,0)),nvl(pr_precision,0)),"
							+" mm_balance=round(mm_oneuseqty*?*(1+ (CASE WHEN ?='OS' AND NVL(pr_exportlossrate,0)>0 THEN NVL(pr_exportlossrate, 0) "
							+" ELSE (case when ?>0 then NVL(pr_exportlossrate, 0) else NVL(pr_lossrate, 0) end ) END )* 0.01)+0.4999*power(0.1,nvl(pr_precision,0)),nvl(pr_precision,0))-mm_oneuseqty*? where mm_maid=? and mm_qty>=mm_oneuseqty*? ",remainmaqty,maketype,osLoss,remainmaqty,maketype,osLoss,remainmaqty,id,remainmaqty);			
			}
	...
	public void enforceEndMake(int ma_id, String caller, String remark) {
		...
		if(baseDao.isDBSetting("usingMakeCraft")){
			baseDao.execute("update MakeCraft set mc_status='" + BaseUtil.getLocalMessage("FINISH") + "',mc_statuscode='FINISH' where mc_maid="+ma_id);
		}
		// 记录操作
	...
	public void turnOSMake(String caller, Integer maid, String kind) {
		...
		//检查是否存在作业单
		int count1=baseDao.getCount("select count(1) from makecraft where mc_makecode=(select ma_code from make where ma_id="+maid+")");
		if(count1>0){
			BaseUtil.showError("当前制造单存在车间作业单或工序委外单，不能转委外，请先删除车间作业单或工序委外单");
		}
		baseDao.updateByCondition("Make", "ma_tasktype='OS',ma_checkstatuscode='UNAPPROVED',ma_checkstatus='未批准',ma_kind='" + kind + "'",
				"ma_id=" + maid);
		baseDao.updateByCondition("makematerial", "mm_mdcode=''", "mm_maid="+maid);
		// 记录操作
	...
	public void turnOSToMake(String caller, Integer maid, String kind) {
		...
		if(baseDao.isDBSetting("usingMakeCraft")){//启用工厂模式  重新计算用料
			// 执行运算存储过程
			Object macode=baseDao.getFieldDataByCondition("make", "ma_code","ma_id="+maid);
			String str = baseDao.callProcedure("MM_MakeMaterialM", new String[] { macode.toString(), SystemSession.getUser().getEm_name() });			
			if (str != null && !str.trim().equals("")) { 
				// 提示错误信息
				BaseUtil.showError("工单：" + macode.toString() + str);
			} 
		}
	}
