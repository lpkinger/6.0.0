...
@Service("BOMTestService")
public class BOMTestServiceImpl implements BOMTestService{
	@Autowired
	private BaseDao baseDao;
	@Autowired
	private HandlerService handlerService;
	@Override
	public void saveBOMTest(String formStore, String gridStore, String caller) {
		...
		//执行保存前的其它逻辑
		handlerService.beforeSave("BOM!Test", new Object[]{formStore, gstore});		//保存
		...
		//执行保存后的其它逻辑
		handlerService.afterSave("BOM!Test", new Object[]{formStore, gstore});
	}
	@Override
	public void deleteBOMTest(int bo_id, String caller) {
		//只能删除在录入的采购单!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.delOnlyEntering(status);
		//执行删除前的其它逻辑
		handlerService.beforeDel("BOM!Test", new Object[]{bo_id});		//删除
		...
		//执行删除后的其它逻辑
		handlerService.afterDel("BOM!Test", new Object[]{bo_id});
	}
	
	@Override
	public void updateBOMTestById(String formStore, String gridStore, String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		List<Map<Object, Object>> gstore = BaseUtil.parseGridStoreToMaps(gridStore);
		//只能修改[在录入]的采购单资料!
		...
		//执行修改前的其它逻辑
		handlerService.beforeSave("BOM!Test",new Object[]{formStore,gstore});
		//修改
	...
	@Override
	public void auditBOMTest(int bo_id, String caller) {
		//只能对状态为[已提交]的订单进行审核操作!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.auditOnlyCommited(status);
		//执行审核前的其它逻辑
		handlerService.beforeAudit("BOM!Test", new Object[]{bo_id});		//执行审核操作
		baseDao.audit("BOM", "bo_id=" + bo_id, "bo_status", "bo_statuscode", "bo_auditdate", "bo_auditman");
		//记录操作
		baseDao.logger.audit(caller, "bo_id", bo_id);
		//执行审核后的其它逻辑
		handlerService.afterAudit("BOM!Test", new Object[]{bo_id});
	}
	@Override
	public void resAuditBOMTest(int bo_id, String caller) {
	...
	@Override
	public void submitBOMTest(int bo_id, String caller) {
		//只能对状态为[在录入]的订单进行提交操作!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.submitOnlyEntering(status);
		//执行提交前的其它逻辑
		handlerService.beforeSubmit("BOM!Test",new Object[]{bo_id});
		//执行提交操作
		baseDao.submit("BOM", "bo_id=" + bo_id, "bo_status", "bo_statuscode");
		//记录操作
		baseDao.logger.submit(caller, "bo_id", bo_id);
		//执行提交后的其它逻辑
		handlerService.afterSubmit("BOM!Test",new Object[]{bo_id});
	}
	@Override
	public void resSubmitBOMTest(int bo_id, String caller) {
