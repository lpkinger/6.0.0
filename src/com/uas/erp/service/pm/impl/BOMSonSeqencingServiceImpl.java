...
@Service("BOMSonSeqencingService")
public class BOMSonSeqencingServiceImpl implements BOMSonSeqencingService{
	@Autowired
	private BaseDao baseDao;
	@Autowired
	private HandlerService handlerService;
	@Override
	public void saveBOMSonSeqencing(String formStore, String gridStore, String caller) {
		...
		//执行保存前的其它逻辑
		handlerService.beforeSave("BOM!SonSeqencing",new Object[]{store, gstore});		//保存
		...
		//执行保存后的其它逻辑
		handlerService.afterSave("BOM!SonSeqencing",new Object[]{store, gstore});
	}
	
	@Override
	public void deleteBOMSonSeqencing(int bo_id, String caller) {
		//只能删除在录入的采购单!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.delOnlyEntering(status);
		//执行删除前的其它逻辑
		handlerService.beforeDel("BOM!SonSeqencing",new Object[]{bo_id});		//删除
		...
		//执行删除后的其它逻辑
		handlerService.afterDel("BOM!SonSeqencing",new Object[]{bo_id});
	}
	
	@Override
	public void updateBOMSonSeqencingById(String formStore, String gridStore, String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		List<Map<Object, Object>> gstore = BaseUtil.parseGridStoreToMaps(gridStore);
		//只能修改[在录入]的采购单资料!
		...
		//执行修改前的其它逻辑
		handlerService.beforeUpdate("BOM!SonSeqencing",new Object[]{store,gstore});
		//修改
		...
		//执行修改后的其它逻辑
		handlerService.afterUpdate("BOM!SonSeqencing",new Object[]{store,gstore});
	}
	
	@Override
	public void auditBOMSonSeqencing(int bo_id, String caller) {
		//只能对状态为[已提交]的订单进行审核操作!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.auditOnlyCommited(status);
		//执行审核前的其它逻辑
		handlerService.beforeAudit("BOM!SonSeqencing", new Object[]{bo_id});		//执行审核操作
		baseDao.audit("BOM", "bo_id=" + bo_id, "bo_status", "bo_statuscode","b_auditdate","bo_auditman");
		//记录操作
		baseDao.logger.audit(caller, "bo_id", bo_id);
		//执行审核后的其它逻辑
		handlerService.afterAudit("BOM!SonSeqencing", new Object[]{bo_id});
	}
	
	@Override
	public void resAuditBOMSonSeqencing(int bo_id, String caller) {
	...
	@Override
	public void submitBOMSonSeqencing(int bo_id, String caller) {
		//只能对状态为[在录入]的订单进行提交操作!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.submitOnlyEntering(status);
		//执行提交前的其它逻辑
		handlerService.beforeSubmit("BOM!SonSeqencing", new Object[]{bo_id});		//执行提交操作
		baseDao.submit("BOM", "bo_id=" + bo_id, "bo_status", "bo_statuscode");
		//记录操作
		baseDao.logger.submit(caller, "bo_id", bo_id);
		//执行提交后的其它逻辑
		handlerService.afterSubmit("BOM!SonSeqencing", new Object[]{bo_id});
	}
	
	@Override
	public void resSubmitBOMSonSeqencing(int bo_id, String caller) {
		//只能对状态为[已提交]的订单进行反提交操作!
		Object status = baseDao.getFieldDataByCondition("BOM", "bo_statuscode", "bo_id=" + bo_id);
		StateAssert.resSubmitOnlyCommited(status);
		handlerService.beforeResSubmit("BOM!SonSeqencing",new Object[]{bo_id});		//执行反提交操作
		baseDao.resOperate("BOM", "bo_id=" + bo_id, "bo_status", "bo_statuscode");
		//记录操作
		baseDao.logger.resSubmit(caller, "bo_id", bo_id);
		handlerService.afterResSubmit("BOM!SonSeqencing",new Object[]{bo_id});
	}
}
