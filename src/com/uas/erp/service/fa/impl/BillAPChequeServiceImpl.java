...
@Service("billAPChequeService")
public class BillAPChequeServiceImpl implements BillAPChequeService {
	@Autowired
	...
	@Override
	public void saveBillAPCheque(String formStore, String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		// 当前编号的记录已经存在,不能新增!
		boolean bool = baseDao.checkByCondition("BillAPCheque", "bar_code='" + store.get("bar_code") + "'");
		if (!bool) {
		...
		// 保存BillAPCheque
		String formSql = SqlUtil.getInsertSqlByFormStore(store, "BillAPCheque", new String[] {}, new Object[] {});
		baseDao.execute(formSql);
		int bar_id = Integer.parseInt(store.get("bar_id").toString());
		baseDao.execute("update BillAPCheque set bar_doublebalance=round(bar_doublebalance,2) where bar_id=" + bar_id);
		baseDao.execute("update BillAPCheque set bar_topaybalance=round(bar_topaybalance,2) where bar_id=" + bar_id);
	...
	@Override
	public void deleteBillAPCheque(int bar_id, String caller) {
		// 只能删除在录入的单据!
		Object[] status = baseDao.getFieldsDataByCondition("BillAPCheque", new String[] { "bar_statuscode", "bar_date" }, "bar_id="
				+ bar_id);
		StateAssert.delOnlyEntering(status[0]);
		// 执行删除前的其它逻辑
		handlerService.beforeDel(caller, bar_id);
		// 删除BillAPCheque
		baseDao.deleteById("BillAPCheque", "bar_id", bar_id);
		// 记录操作
	...
	@Override
	public void updateBillAPChequeById(String formStore, String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		// 只能修改[在录入]的资料!
		Object status = baseDao.getFieldDataByCondition("BillAPCheque", "bar_statuscode", "bar_id=" + store.get("bar_id"));
		StateAssert.updateOnlyEntering(status);
		// 执行修改前的其它逻辑
		handlerService.beforeUpdate(caller, new Object[] { store });
		// 修改Inquiry
		String formSql = SqlUtil.getUpdateSqlByFormStore(store, "BillAPCheque", "bar_id");
		baseDao.execute(formSql);
		baseDao.execute("update BillAPCheque set bar_doublebalance=round(bar_doublebalance,2) where bar_id=" + store.get("bar_id"));
		baseDao.execute("update BillAPCheque set bar_topaybalance=round(bar_topaybalance,2) where bar_id=" + store.get("bar_id"));
		// 记录操作
	...
	@Override
	public void printBillAPCheque(int bar_id, String caller) {
		// 只能打印审核后的单据!
		Object status = baseDao.getFieldDataByCondition("BillAPCheque", "bar_statuscode", "bar_id=" + bar_id);
	...
	@Override
	public void auditBillAPCheque(int bar_id, String caller) {
		// 只能对状态为[已提交]的订单进行审核操作!
		Object[] status = baseDao.getFieldsDataByCondition("BillAPCheque", new String[] { "bar_statuscode", "bar_billkind", "bar_custcode",
		...
		// 执行审核操作
		baseDao.audit("BillAPCheque", "bar_id=" + bar_id, "bar_status", "bar_statuscode", "bar_auditdate", "bar_auditer");
		// 记录操作
	...
	@Override
	public void resAuditBillAPCheque(int bar_id, String caller) {
		// 只能对状态为[已审核]的订单进行反审核操作!
		Object[] status = baseDao.getFieldsDataByCondition("BillAPCheque", new String[] { "bar_statuscode", "bar_code", "bar_kind",
		...
		// 执行反审核操作
		baseDao.resAudit("BillAPCheque", "bar_id=" + bar_id, "bar_status", "bar_statuscode", "bar_auditdate", "bar_auditer");
		baseDao.execute("update BillAPCheque set bar_checkno=null where bar_id=" + bar_id);
		// 记录操作
	...
	private void checkCode(Object barcode, Object checkcode) {
		Object code = baseDao.getFieldDataByCondition("BillAPCheque", "bar_code", "bar_code <> '" + barcode + "' AND bar_checkcode='"
				+ checkcode + "'");
		if (code != null) {
	...
	@Override
	public void submitBillAPCheque(int bar_id, String caller) {
		...
		baseDao.execute("update BillAPCheque set bar_doublebalance=round(bar_doublebalance,2) where bar_id=" + bar_id);
		baseDao.execute("update BillAPCheque set bar_topaybalance=round(bar_topaybalance,2) where bar_id=" + bar_id);
		boolean bool = baseDao.checkIf("BillAPCheque", "bar_id=" + bar_id
		...
		// 执行提交操作
		baseDao.updateByCondition("BillAPCheque", "bar_statuscode='COMMITED',bar_status='" + BaseUtil.getLocalMessage("COMMITED") + "'",
				"bar_id=" + bar_id);
		// 记录操作
	...
	@Override
	public void resSubmitBillAPCheque(int bar_id, String caller) {
		...
		// 执行反提交操作
		baseDao.updateByCondition("BillAPCheque", "bar_statuscode='ENTERING',bar_status='" + BaseUtil.getLocalMessage("ENTERING") + "'",
				"bar_id=" + bar_id);
		// 记录操作
	 ...
	 * 复制应收票据
	 */
	public JSONObject copyBillAPCheque(int id, String caller) {
		Map<String, Object> dif = new HashMap<String, Object>();
		// Copy
		int nId = baseDao.getSeqId("BillAPCheque_SEQ");
		String code = baseDao.sGetMaxNumber("BillAPCheque", 2);
		dif.put("bar_id", nId);
		...
		baseDao.copyRecord("BillAPCheque", "BillAPCheque", "bar_id=" + id, dif);
		JSONObject obj = new JSONObject();
		obj.put("bar_id", nId);
		obj.put("bar_code", code);
		baseDao.execute("update BillAPCheque set bar_leftamount=nvl(bar_doublebalance,0) where bar_id=" + nId);
		baseDao.logger.others("单据复制" + code, "复制成功", "BillAPCheque", "bar_id", id);
		return obj;
	}

	@Override
	public void updateInfo(int id, String text, String caller) {
		baseDao.updateByCondition("BillAPCheque", "bar_duedate=to_date('" + text + "','yyyy-mm-dd')", "bar_id=" + id);
		// 记录操作
	...
	@Override
	public void endBillAPCheque(int bar_id, String reason, String caller) {
		baseDao.updateByCondition("BillAPCheque", "BAR_CHANGEREASON='" + reason + "', bar_changedate=sysdate", "bar_id=" + bar_id);
		// 执行结案操作
		baseDao.execute("update BillAPCheque set bar_status='已结案', bar_statuscode='FINISH' where bar_id=" + bar_id);
		// 记录操作
		baseDao.logger.end(caller, "bar_id", bar_id);
	}

	@Override
	public void resEndBillAPCheque(int bar_id, String caller) {
		// 执行反结案操作
		baseDao.execute("update BillAPCheque set bar_status='已审核', bar_statuscode='AUDITED',BAR_CHANGEREASON=null,bar_changedate=null where bar_id="
				+ bar_id);
		// 记录操作
	...
	public String turnAccountRegister(int bar_id, String accountcode, String caller) {
		...
		String dets = baseDao.getJdbcTemplate().queryForObject(
				"select wmsys.wm_concat(ar_code) from AccountRegister where ar_sourcetype='应付支票' and ar_sourceid=?", String.class, bar_id);
		if (dets != null) {
		...
		if (StringUtil.hasText(accountcode)) {
			String error = baseDao
					.getJdbcTemplate()
					.queryForObject(
							"select wmsys.wm_concat(ca_code) from Category where ca_code=? and ca_code not in (select ca_code from Category where (abs(nvl(ca_iscash,0))=1 OR abs(nvl(ca_isbank,0))=1) and nvl(ca_isleaf,0)<>0 and nvl(ca_statuscode,' ')='AUDITED')",
							String.class, accountcode);
			if (error != null) {
				BaseUtil.showError("填写科目编号不存在，或者状态不等于已审核，或者不是末级科目，或者不是银行现金科目，不允许转银行登记！");
			}
			baseDao.execute("update BillAPCheque set bar_accountcode=? where bar_id=?", accountcode, bar_id);
			baseDao.execute(
					"update BillAPCheque set bar_accountname=(select ca_description from category where ca_code=bar_accountcode) where bar_id=?",
					bar_id);
		}
