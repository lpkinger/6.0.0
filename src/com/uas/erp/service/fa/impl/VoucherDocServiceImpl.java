...
public class VoucherDocServiceImpl implements VoucherDocService {
	@Autowired
	...
	@Override
	public void saveVoucherDoc(String formStore, String gridStore, String caller) {
		...
		if (gridStore.contains("},")) {// 明细行有多行数据哦
			...
			for (int i = 0; i < datas.length; i++) {
				vd_id[i] = baseDao.getSeqId("VOUCHERDOC_SEQ");
			}
		} else {
			vd_id[0] = baseDao.getSeqId("VOUCHERDOC_SEQ");
		}
		List<String> gridSql = SqlUtil.getInsertSqlbyGridStore(gridStore,
				"VoucherDoc", "vd_id", vd_id);
		...
		// 执行保存后的其它逻辑
		handlerService.handler("VoucherDoc", "save", "after",
				new Object[] { formStore });
	}

	@Override
	public void deleteVoucherDoc(int vo_id, String caller) {
		// 只能删除在录入的单据!
		...
		// 删除DispatchDetail
		baseDao.deleteById("VoucherDoc", "vd_voucherid", vo_id);
		// 记录操作
	...
	@Override
	public void updateVoucherDocById(String formStore, String gridStore,
			String caller) {
		Map<Object, Object> store = BaseUtil.parseFormStoreToMap(formStore);
		List<Map<Object, Object>> gstore = BaseUtil
				.parseGridStoreToMaps(gridStore);
		...
		for (Map<Object, Object> s : gstore) {
			if (s.get("vd_id") == null || s.get("vd_id").equals("")
				...
				int id = baseDao.getSeqId("VOUCHERDOC_SEQ");
				String sql = SqlUtil.getInsertSqlByMap(s, "VoucherDoc",
	...
	@Override
	public void auditVoucherDoc(int vo_id, String caller) {
	...
	@Override
	public void resAuditVoucherDoc(int vo_id, String caller) {
	...
	@Override
	public void submitVoucherDoc(int vo_id, String caller) {
	...
	@Override
	public void resSubmitVoucherDoc(int vo_id, String caller) {
