...
@Service
public class PaymentsSaleServiceImpl implements PaymentsSaleService{
	@Autowired
	...
	public void savePayments(String formStore, String gridStore, String caller) {
		...
		//当前编号的记录已经存在,不能新增!
		boolean bool = baseDao.checkByCondition("Payments", "pa_code='" + store.get("pa_code") + "' and nvl(pa_class,' ')='收款方式'");
		if(!bool){
		...
		for(Map<Object, Object> g:grid ){
			if(!StringUtil.hasText(g.get("pad_kind"))){
				BaseUtil.showError("请填写业务类型");
			}
		}
		//保存
		store.put("pa_class", "收款方式");
