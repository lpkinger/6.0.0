...
public class BatchQuantityServiceImpl implements BatchQuantityService{
	...
	public void updateBatchById(String formStore) {
		...
		//执行修改前的其它逻辑
		handlerService.handler("Batch!Quantity", "save", "before", new Object[]{store});
		//修改
		String formSql = SqlUtil.getUpdateSqlByFormStore(store, "Batch", "ba_id");
		baseDao.execute(formSql);
		//记录操作
		baseDao.logger.update("Batch!Quantity", "ba_id", store.get("ba_id"));
		//执行修改后的其它逻辑
		handlerService.handler("Batch!Quantity", "save", "after", new Object[]{store});
	}
}
